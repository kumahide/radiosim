...
- 🔴 **Over rugged terrain (mountain paths) the diffraction loss diverges and the result is unusable** (known defect, fix planned). A broad ridge is counted as many independent knife edges, so diffraction loss comes out at **hundreds to thousands of dB** and the Fresnel-zone blockage far exceeds **100%**. The verdict will read NG, but **you cannot tell from these numbers whether that NG is correct**. ⇒ **Trust the results only on flat or coastal paths with roughly 25 m of relief or less** (measured). The ±5–15 dB figure above applies within that range
