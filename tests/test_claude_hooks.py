...
# ============================================================
# 状態語が語彙の外＝項目が丸ごと注入から消える（2026-09-09・I-129 で実際に踏んだ）
# ============================================================
#
# 🔴 **2026-08-05 の事故（上の「壊れ方1〜3」）と同じ実害が、別の扉から再発した。**
# あのとき塞いだのは*強調記号と同義語*で、`normalize_state` は「読めた語」を正しく
...
# ⇒ 語彙を足して塞ぐと次の語でまた開く。**「読めなかったこと自体を鳴らす」**へ昇格した
...
class TestUnreadableState:
    ...
    def test_壊れ方1_語彙の外の状態語で鳴ること(self, hook, written):
        """①語彙外の語が書かれたら必ず鳴ること（＝一度も落ちないゲートにしない）。"""
        doc = _doc("## 🐞 バグ", _item("I-129", written))
        assert hook.unreadable_state_items(doc) == ["I-129(語なし)"], \
            f"{written!r} が語彙の外なのに鳴っていない"

    def test_壊れ方1_症状そのものも測る(self, hook):
        ...
        """
        ok = _doc("## 🐞 バグ", _item("I-129", "保留（再現待ち）"))
        ng = _doc("## 🐞 バグ", _item("I-129", "**未解決**"))
        assert len(hook.parse_issues(ok)[0]) == 1
        assert len(hook.parse_issues(ng)[0]) == 0, \
            "語彙外でも注入に載るなら、この事故は起きていないはず（前提が崩れている）"
    ...
    def test_壊れ方2_正しく書けている項目で鳴らないこと(self, hook, written):
        """②毎回鳴る網にしない（凡例の 5 語＋同義語＋註釈つきは全部通る）。"""
        doc = _doc("## 🐞 バグ", _item("B-004", written, resp="`abc1234`"))
        assert hook.unreadable_state_items(doc) == [], f"{written!r} で誤検知している"

    def test_壊れ方3_状態欄そのものが無い項目も同じ扱い(self, hook):
        """③要求しているものが正しいか＝見ているのは「語」ではなく**読めるか**。

        ★必須の状態欄が丸ごと無い項目も、結果は同一（項目が注入から消える）。
        語だけを見る実装だとここが素通りする＝*間違ったものを要求している*ゲートになる。
        """
    ...
    def test_記入例テンプレでは鳴らないこと(self, hook):
        """テンプレの見出しは `B-0XX`（数字でない）＝最初から拾わない。

        ⚠️ ここが鳴ると**毎セッション必ず鳴る**（テンプレは常にファイルの中にいる）＝
        壊れ方②に直行する。`parse_issues` が同じ理由で `\\d{3}` を使っているのと同じ形。
        """
        doc = _doc("### ★ B-0XX: （症状を一言で）\n\n- ★ **状態**: 未着手 / 対応中 / 済\n")
        assert hook.unreadable_state_items(doc) == []

    def test_実台帳が今この穴を持っていないこと(self, hook):
        """実データ回帰＝いま `ISSUES.md` に読めない状態欄が 1 件も無いこと。

        ⚠️ 他の実データテストと同じく、CI では `ISSUES.md` が存在せず skip になる
