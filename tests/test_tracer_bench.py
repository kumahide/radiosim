"""
tests/test_tracer_bench.py
==========================
実測補助アプリ（`apps/tracer`）の**段0 の机上試験**（増分6）を検証する。
...
from apps.tracer import bench as B
from apps.tracer import bench_analysis as BA
from apps.tracer import calib as C
...
class Device:
    ...
    def command(self, line: str) -> None:
        ...
        if parts == ["show"]:
            self.text("OK")
            self.buffer += self.frame("TRACER_CONFIG", self.config_values())
            return
        if parts[0] == "cont":
            seconds = int(parts[1])
            self.config_id += 1
    ...
    def tick(self, other: "Device") -> None:
        ...
        if t >= self.next_air_config:
            if other.hears(self):
                other.buffer += self.frame("TRACER_CONFIG", self.config_values())
            self.next_air_config = t + 1.0
        if t >= self.next_packet:
            self.buffer += self.frame("TRACER_TX_PACKET", dict(
    ...
    def receive(self, tx: "Device") -> None:
        ...
        self.buffer += self.frame("TRACER_RX_SAMPLE", dict(
...
def _config_message(dev: Device, **over) -> R.Message:
    ...
    values.update(over)
    frame = dev.frame("TRACER_CONFIG", values)
...
def test_the_continuous_report_is_read_as_the_firmware_writes_it():
    ...
    radio = (Path(__file__).resolve().parents[1] / "apps" / "tracer" / "firmware" / "main"
...
def test_the_continuous_mode_uses_the_measurement_transmit_path():
    ...
    main_dir = Path(__file__).resolve().parents[1] / "apps" / "tracer" / "firmware" / "main"
    ...
    assert "TRACER_AIR_CONT_TAG" in body and "TRACER_AIR_TAG" not in body
    ...
    assert "TRACER_ROLE_TX" in handler
...
def test_text_lines_are_picked_out_between_frames_in_any_split(dialect):
    ...
    data = (b"OK config_id=3 \xe5\x86\x8d\xe8\xb5\xb7\xe5\x8b\x95\r\n"
            + dev.frame("TRACER_CONFIG", dev.config_values())
