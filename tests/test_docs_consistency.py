...
#       説明が十分か）は独立レビュー①（Codex・`/release` 工程 4b）に委ねる（機械化
#       できる列挙のみ守る）。⚠️ 2026-08-31 まではローカル LLM の doc-review 助言を
#       受け皿と書いていたが、真陽性が導入時の 1 件だけで廃止した。
...
#
# ここで守るのは**開示が全数に在ること**だけ（文面の良し悪しは独立レビュー①＝Codex へ）。
# ✅ **2026-08-25（3.0）に発散そのものを塞いだので、開示すべき中身を書き換えた**
#   （予告どおり**ゲートは消さない**＝頭打ちの説明も残る）。塞いだのは「広がりのある
#   障害物を多数の稜線として数える」ことだけで、**過大に出る可能性は残る**:
#     ①複数の峰にまたがる合成損失は**真値と照合していない**（系統の違う 2 手法と
#       桁を並べただけ・手法の大小に一般的な順序は無い）
#     ②深く遮蔽された経路では**標本数を増やすと回折損が増える**（+18%・旧 +92%）
#   ⇒ この 2 つを必須の目印に足し、**言い過ぎを禁じる否定側**を対で置く（下）。
...
# ⇒ **数値の境界は目印にしない。**開示すべき概念は 5 つ＝①過大に出ること
...
_FORBIDDEN_SAFE_BOUNDARY = [
    ...
    # 🆕 **3.0 の修正そのものが新しい「ここまでは大丈夫」を生む**（2026-08-25）。
    #   実測は「不連続な跳びは消えるが、端点近傍の最大 ν が標本間隔に依存する分は
    #   残る」＝**+18% は残っている**。「標本数に安定」「発散しなくなった」と
    #   言い切るのは実測に反する（→ [[feedback-promote-recurring-checks]]）。
    r"標本数に(は)?安定",
    r"サンプル数に(は)?安定",
    r"発散(は)?しなくなり",
    r"発散する経路は(もう)?ありません",
    r"stable (with respect to|across) (the )?sample",
    r"no longer diverges?",
]
