"""
tests/test_field_recorder.py
=============================
実測補助アプリ（`apps/field`）の**測りながら書く部分**と CLI を検証する（増分4）。
...
フレームの並び・CRC_EXTRA の正しさは `test_field_mavlink.py` が仕様から手で
...
def _sample_frame(
    ...
    return _frame(dialect, "RADIOSIM_FIELD_RX_SAMPLE", payload, seq)
...
def _config_frame(
    ...
    return _frame(dialect, "RADIOSIM_FIELD_CONFIG", payload)
