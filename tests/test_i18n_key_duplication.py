...
2 キーで持たれているのが見つかった（B-053 で ↻ の置き場を 3 ウィンドウで揃えたときに発覚）。
**置き場は揃ったのに出所は揃っていない**状態で、片方だけ直せば画面に 2 通りの語が出る。

⇒ 全数を数えたら**画面どうしの重複は 11 組・24 キー**あった（I-073）。ウィンドウの名前が
ランチャーのボタン・ウィンドウの題・ダイアログの題で 3 キーに分かれている類が主で、
...
⚠️ **例外は 1 つの系統だけ**＝`col_*`（表の列見出し）と `lbl_b_*`（共通設定のバー）は
...
#: `col_*`＝表の列見出し／`lbl_b_*`＝共通設定のバー。どちらも ja は本体と同じ語のまま、
