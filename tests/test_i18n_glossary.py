...
# ============================================================
# 4. 日本語の画面語彙が「○○＝△△」の説明形を使っていないこと（I-150）
# ------------------------------------------------------------
# 🔴 2026-09-13・ユーザー指摘「○○○○＝△△△△△という説明は筋が悪い」＝
# 3.4 で足した 6 キー（回折の disclosure・解像度の disclosure・地面反射の軸名）
# を自然な日本語へ書き直した。**新しく足すときにも同じ穴が開く**ので、
# 個々の文言ではなく**この記号そのもの**を締め出す（英語には無い問題＝
# `en` テーブルは対象外）。
# ============================================================
def test_japanese_ui_text_never_uses_the_equals_explanation_form():
    offenders = [f"{key}: {shown}" for lang, key, shown in _shown_strings()
                 if lang == "ja" and "＝" in shown]
    assert not offenders, (
        "日本語の画面語彙に「○○＝△△」の説明形が残っている"
        "（自然な日本語の文へ書き直すこと・I-150）: " + ", ".join(offenders)
    )
