...
def test_colorref_reverses_byte_order():
    ...
    assert theme._colorref("#112233") == 0x00332211
    assert theme._colorref("#ff0000") == 0x000000ff
    assert theme._colorref("#00ff00") == 0x0000ff00
    assert theme._colorref("#0000ff") == 0x00ff0000
...
def test_decorated_hwnd_asks_get_parent_not_winfo_id(root):
    ...
    hwnd = theme._decorated_hwnd(root, fake)
    assert hwnd == 9999
...
def test_apply_title_bar_theme_sends_dark_mode_to_the_decorated_hwnd(root):
    ...
    ok = theme.apply_title_bar_theme(root, fake)
    assert ok is True
    ...
    assert attr == theme._DWMWA_USE_IMMERSIVE_DARK_MODE
...
def test_apply_title_bar_theme_falls_back_to_the_old_attribute_number(root):
    ...
    class _OldOnlyDwmapi:
        ...
        def DwmSetWindowAttribute(self, hwnd, attr, value_ref, size):
            ...
            return 0 if attr == theme._DWMWA_USE_IMMERSIVE_DARK_MODE_OLD else 1

    fake = _FakeDwmDll(parent_hwnd=1)
    fake.dwmapi = _OldOnlyDwmapi()
    ok = theme.apply_title_bar_theme(root, fake)
    assert ok is True
    assert theme._DWMWA_USE_IMMERSIVE_DARK_MODE in fake.dwmapi.attrs_tried
    assert theme._DWMWA_USE_IMMERSIVE_DARK_MODE_OLD in fake.dwmapi.attrs_tried
...
def test_apply_title_bars_covers_root_and_open_toplevels(root):
    ...
    """
    seen: list[tk.Misc] = []
    original = theme.apply_title_bar_theme
    ...
    try:
        theme.apply_title_bar_theme = spy
        try:
            theme.apply_title_bars(root)
        finally:
            theme.apply_title_bar_theme = original
