...
| 地形の解像度 | 地形を刻む細かさの段階（高＝約 5m / 中＝約 10m / 低＝約 20m 間隔）。**利用者が選ぶのは段階だけ**で、点数は経路の長さから決まる。 *The step at which the terrain is sampled (High ≈ 5 m / Medium ≈ 10 m / Low ≈ 20 m). The user picks the step only; the number of points follows from the path length.* | Terrain Resolution | 地形サンプル数 / 分解能 |
| サンプル数 | 経路上で実際に標高を取った点の数。**解像度の段階と経路の長さから決まる**結果であって、入力ではない。 *How many points along the path an elevation was actually taken at — a result of the resolution step and the path length, not an input.* | Samples | 標本数 |
