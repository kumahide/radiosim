...
| 地形の解像度 | 地形を刻む細かさの段階（高＝5m 層 / 中＝10m 層 / 低＝20m 間隔）。**利用者が選ぶのは段階だけ**で、点数はアプリが解く。⚠️ 「高」「中」は**その層の DEM 画素の縁ごと**に標本を置く＝**等間隔ではない**（「低」だけ 20m の等間隔）。 *The step at which the terrain is sampled (High = 5 m layer / Medium = 10 m layer / Low = 20 m spacing). The user picks the step only; the app resolves the points. "High" and "medium" sample at the edges of each DEM pixel — not evenly spaced.* | Terrain Resolution | 地形サンプル数 / 分解能 |
