...
│       ├── bench.py      # Stage-0 bench test (swaps the two units' roles by command, measures both directions, keeps raw logs and steps)
│       ├── bench_analysis.py # Bench-test evaluation (recounts from the raw logs: linearity, dither, receive rate, calibration files)
│       ├── cli.py        # Command line (header template, record, export to the batch CSV, bench test, continuous transmit)
