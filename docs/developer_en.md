...
- **Pick Coordinates mode (default)**: click the map to fill **whichever site is still empty** (TX, then RX) and write it back to the launcher's start/end fields (the numeric fields are the source of truth). **Once both are set a plain click writes nothing** — from there you select a point and move it (below). Wired via `apply_map_pick` / `current_path_coords`.
