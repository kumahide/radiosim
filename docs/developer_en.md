...
> - **The `status` column becomes multi-valued.** Today, `status` in `summary.csv` / `hops.csv` / `scenario.csv` is a 2-value field: **OK** (margin ≥ 0 dB) / **NG** (< 0 dB). In 4.0, the sensitivity calculation introduced in 3.4 (`core/sensitivity.py` — a recalculation across pessimistic/optimistic assumptions for DEM elevation, vegetation height, environment class, diffraction model, and so on) will feed into the verdict, giving four values: **OK** (holds even under pessimistic assumptions) / **NG** (fails even under optimistic assumptions) / **REVIEW** (flips depending on assumptions — a site visit is recommended) / **ERR** (could not be computed — missing data). This replaces `core/models.py`'s `status = "OK" if actual_margin >= 0 else "NG"`. The column name, position, and the OK/NG thresholds themselves do not change. The new values and the conditions for each will be spelled out in the 4.0 CHANGELOG.
