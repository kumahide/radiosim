...
**★クラス点検の単位**でもあり、欠陥を直すときは「この 4 つ＋地図の 5 面すべてに当てはまらないか」を必ず問う。

| フロー | 実装 | 入力の source of truth |
| --- | --- | --- |
| **個別シミュレーション** | `simulation.py` | ランチャーの入力欄 |
| **複数経路** | `batch.py` | バッチ表（CSV I/O） |
| **条件探索（比較 / スイープ）** | `scenario.py` | 固定した経路＋条件列。**DEM 取得 1 回＋純計算 N 回**（計算が 2 フェーズであることを使う） |
| **中継経路** | `multihop.py` | waypoint 列（地点）。区間は導出。再生中継モデルで**全体判定は min**（最も余裕の少ない区間）、内訳と両方を出力 |

#### 地図

- **地図**（`views/map_window.py`・アプリ唯一のインスタンス／4 モード）: 座標入力／中継点の指定／複数経路への連続追加／DEM キャッシュの可視化・事前取得・削除
...
- **A4 レポート（v2）**: per-path / summary を A4 縦の印刷確定枠で出力（`@page A4`＋Ctrl+P でゼロ依存 PDF 化・自己同定ヘッダ/フッタ）。⚠️ **1 枚に収まるのは per-path だけで**、台帳は改ページする（per-path の縮小フィットは表が切れるので台帳には掛けられない）
- **アンテナ初期指向 AZ/EL**: 相手局へ正対する真方位・仰角を per-path レポートに両端ぶん表示（既存データからの幾何計算による初期値）
...
- 日本語 / 英語 UI 切替（`i18n.py` が単一ソース）＋**利用者が言語を足せる**（`lang/<コード>.json`）。`t()` は前からキー単位で英語へフォールバックするので、外部の表は**上書きできた分だけ**効く。⛔ プラグイン機構は作らない（読むのは JSON 1 種類だけ）。⚠️ **成果物の語（`html_*` / `pl_*`）は開かない**。出力契約が利用者ごとに変わるためである。**差し込み（`{n}` など）が英語と違うキーは採用せず、そのことを起動時に画面で言う**（採用すると `str.format` が `KeyError` を投げてその画面で落ちるため、黙って壊れさせない）
...
- Python 3.11 以上（開発・CI は 3.14 で検証）。⚠️ **配布ビルドには Tk 8.6 を同梱する Python が要ります**。3.14.7 のように Tk 9 を同梱する版で作った exe は起動しないため、`build.bat` が Tk の版を見て止めます（`python -c "import tkinter; print(tkinter.TkVersion)"` で確かめられます）
...
- **残すもの**: 仮想環境と、**リポジトリ直下の** `terrain_cache/` `results/` `basemap_pale/`（ソース実行で貯めたデータで、取り直しが高価、または利用者のもの）
...
`build.bat` と同じビルドを行ったうえで、[installer/radiosim.iss](../installer/radiosim.iss)（Inno Setup）を使って `RadioSimPro-Setup-<版>.exe` を生成します。**`portable.txt` は同梱しません**（インストール先は常に非ポータブル扱いで、設定・キャッシュ・ログ・結果は OS 標準の場所）。
...
- **前提条件**: [Inno Setup 7](https://jrsoftware.org/isinfo.php) 導入済み（`winget install JRSoftware.InnoSetup.7`）＋環境変数 `RADIOSIM_ISCC` に `ISCC.exe` のフルパスを設定。導入先はユーザー単位なら `%LOCALAPPDATA%\Programs\Inno Setup 7\ISCC.exe`、マシン単位なら `C:\Program Files (x86)\Inno Setup 7\ISCC.exe` です（設定例は cmd なら `setx RADIOSIM_ISCC "C:\...\ISCC.exe"`、PowerShell なら `setx RADIOSIM_ISCC "$env:LOCALAPPDATA\Programs\Inno Setup 7\ISCC.exe"`。**`%LOCALAPPDATA%` は PowerShell では展開されない**ので、そのまま貼るとリテラルが保存されます）。未設定・実体なしならビルドを中止します。
  - **配布ビルドは Inno Setup 7 系で作ります**。`build.bat` がコンパイラの版を確かめ、7 でなければ中止します。`radiosim.iss` は 6.7 系でもそのままコンパイルできますが（7 で廃止された機能を 1 つも使っていないため）、**黙って 6 で作った配布物が出てしまう**のを防ぐためのゲートです（上流の更新は 6.7.3〔2026-05-26〕を最後に 7 系へ移っています）。意図して別の版で作るときだけ `RADIOSIM_ISCC_ANY_VERSION=1` を宣言してください。
- **インストール先の既定**: `{autopf}`（`PrivilegesRequired=lowest`。管理者権限が無ければユーザー配下、あればユーザーが選べる）。
- **出力先**: ビルド出力と同じ `dist/`（`RadioSimPro-Setup-<版>.exe`）。
- **署名**: `RADIOSIM_SIGNTOOL` が定義済みなら、exe 本体と同じ 1 か所の呼び出しでインストーラにも署名します（未定義なら未署名のまま）。現在の配布物は未署名です。⚠️ **「未署名では導入がブロックされる」状況は 2026-09-07 に実際に確認しました。** Windows 11 の Smart App Control（⚠️ **クリーンインストールした環境に限らない**・対象地域は限られる）が、インストーラが `%TEMP%` へ展開する本体を実行拒否します（上のトラブルシューティング表）。**証明書の取得は見送ると決定しました（2026-09-11）。** 自己署名では通らず（Smart App Control は信頼された署名者か評価を要求します）、無償の OV 系サービス（SignPath Foundation）や EV も文書上は通る見込みが立ちましたが、単独開発者が毎リリース外部依存を1つ増やすコストに見合わないと判断したためです。ポータブル版 ZIP の案内が恒久的な仕様です。
...
| インストーラが「エラー 4551: アプリケーション制御ポリシーによってこのファイルがブロックされました」で中止（配布先） | **SmartScreen ではなく実行拒否で続行不可**。Windows 11 の Smart App Control（⚠️ **クリーンインストールした環境に限らない**。有効になる経路は複数あり、**そこは数えない**。判定に要るのは「その機械で今オンかどうか」だけ）または組織のアプリケーション制御が、未署名の `setup.tmp`（Inno Setup が `%TEMP%` へ展開する本体）を弾いている。**MOTW ではないので「ブロックの解除」も効かない。** 恒久的な案内はポータブル版 ZIP（署名は見送りと決定済み）。詳細は `docs/manual_ja.md` の「インストーラが Windows に止められたとき」 |
| 上記が自分の環境で起きているか確かめたい | `Get-WinEvent -LogName Microsoft-Windows-CodeIntegrity/Operational` の EventID `3033` / `3077` にブロックされたファイル名とポリシー ID が残る。Smart App Control の状態は `HKLM\SYSTEM\CurrentControlSet\Control\CI\Policy` の `VerifiedAndReputablePolicyState`（`0`=オフ / `1`=オン・適用 / `2`=評価モード） |
...
**版は [`requirements.txt`](../requirements.txt) が単一ソース**です（固定することでソース実行と配布バイナリで同じ版が入る）。名前を並べて入れると版が固定されず、下表と二重管理になります。
...
│   ├── scenario.py       # 条件探索の共有ランナー（A-1 比較 / A-2 スイープ・フェーズの宣言・ヘッドレス）
...
│   ├── map_adapter.py    # tkintermapview への唯一の依存点（防火扉・3.3 ステージ2）
...
│   ├── title_bar.py      # タイトルバー・ウィンドウ枠（Windows が描く領域）の配色を DWM へ申告する
│   ├── window_fit.py     # ウィンドウの寸法と位置を決める唯一の実装（見切れ・画面外防止）
│   ├── window_scroll.py  # 入らないときの逃げ道（中身が画面より大きいウィンドウのスクロール受け皿）
│   ├── scenario.py       # 条件探索ウィンドウ（比較 / スイープ）
│   ├── multihop.py       # 中継経路ウィンドウ（地点が入力面・区間は導出）
│   ├── multihop_map.py   # 中継経路と地図の受け渡し（地点列の写し・地図からの追加/置き直し・Mixin）
│   ├── frozen_common.py  # 凍結バー「共通設定」に何を出すかの単一ソース（複数経路・中継経路）
...
│   └── images/           # そのスクショと図（architecture_*.svg は層構成図）。exe にも同梱
├── README.md             # リポジトリの入口（GitHub で最初に読まれる 1 本）
│
│   # テストは**名前だけ**。説明は「テスト」セクションの表が正典（同じ説明を 2 か所で持たない）。
...
└── tests/
    ...
    ├── test_ground_reflection.py  # 地面反射の振幅包絡線（3.4 ステージ4）
    ├── test_sensitivity.py    # 摂動再計算エンジン（3.4 ステージ4）
    ├── test_residuals.py      # 実測との残差計算（3.4 ステージ5）
    ├── test_report_residuals.py  # PathResult → 残差標本の抽出（3.4 ステージ5）
    ├── test_report_sensitivity.py  # 感度表/残差表の帳票配線（3.4 ステージ6）
    ├── test_simulation.py
    ├── test_config.py
    ├── test_dem.py
    ├── test_dem_sources.py
...
メニューバーは **ファイル / 設定 / ヘルプ** の 3 つ（実装は[views/launcher_menu.py](../views/launcher_menu.py) の `_build_menu`）。ラベルは `i18n.py` の辞書が単一ソース。以下がその全項目です。
...
| 結果フォルダを開く    | `results/` をエクスプローラーで開く（無ければ作ってから開く）            |
...
| 言語                  | 日本語 / English（＋自分で足した言語） | UI の表示言語（再起動で反映）                                   |
| 訳のひな形を書き出す... | —                         | 言語メニューの中の項目。訳せる全キー＋英語の値を JSON で書き出す → [表示言語を自分で足す](../docs/manual_ja.md#表示言語を自分で足す) |
| 座標の表示形式              | 十進度（DD）/ 度分秒（DMS） | 座標の**表示**表記（`coords.py`）→ 下記                        |
| プロキシ設定...       | URL 入力                    | HTTP プロキシを明示指定（空欄 = OS のプロキシ設定を使用）→ 下記 |
| アプリ設定を読込む... | —                           | 設定ファイルからテーマ・言語・プロキシ・座標表示形式**だけ**を取り込む        |
...
| 起動時に更新を確認 | 既定オフ（`update_check_auto`＝`"on"` のときだけ）。起動時に `update_check.auto_due()` が真なら、試みた日（`update_check_last`）を**打つ前に**保存して 1 回問い合わせる＝失敗しても同じ日に 2 度目は打たない。新しい版があるときだけ手動と同じ確認（止め方の 1 文つき）を出し、最新・失敗はログだけ。待つ間に手動で押すと、その答えを手動と同じに出す |
| プレリリースも知らせる | `update_check_prerelease`＝`"on"`／`"off"` は利用者が選んだ値、既定の `""` は「いまの版で決まる」（`update_check.want_prerelease()`＝正式ならオフ・RC/a/b ならオン）。チェックの印はその解いた値を映す。触ったら以後は `"on"`／`"off"` に固定。画面のスレッドで読んで `check(include_pre=...)` へ渡す＝手動と起動時の両方に効く |
...
> **地図はメニューに無く**、ランチャー下部の**「地図」ボタン**から開きます（→ [地図](#地図)）。

### 座標の表示形式（DD / DMS）

**表記（表示）の設定であって入力モードではありません。** 入力は `coords.parse_pair` が DD / DMS の双方を寛容に受理し、計算前に必ず DD へ正規化されます（`simulation.SimParams` へ渡る値は常に DD）。

欄の整形が走るのは**起動時・表記の切替時・設定/プロジェクトの読込時・入力の確定（Enter / focus 離脱）**。⚠️ **入力できる座標欄 3 面はすべて確定でも整形する**（ランチャー `views/launcher.py`・複数経路の表 `views/batch_table.py`・中継経路 `views/multihop.py`。2.7 で揃えた）。**整形されること自体が parse 成功の返事**で、読めなければ原文が残る。⚠️ 条件探索（`views/scenario.py`）の座標は**読み取り専用**（ランチャーの凍結スナップショット）なので対象外で、`_update_path_label` の再描画でしか整形されない。
...
ランチャーの**「地図」ボタン**（`views/map_window.py`）で、国土地理院の淡色地図を使う補助ウィンドウを開きます。**地図はアプリ唯一のインスタンス（ランチャー所有）**で、上部のモードセレクタで 4 モードを切り替えます。コアのシミュレーションは地図なしで完結し、地図は便利機能です。開いたとき設定中 TX/RX の経路長に合わせて自動ズーム・センタリングします。⚠️ **複数経路のウィンドウからも地図を開けます**（「地図から選択」で連続追加モードを開く。2.7 で追加。それまでは地図の側からしか連続追加を始められなかった）。

**描き方は 4 モードで 1 つの規則**。**線 ＋ 端点アイコン（TX=塗り / RX=白抜き・中継点=ひし形）＋ 中点に水平距離バッジ**。⚠️ **モードごとに描き分けない**（2026-08-14 にユーザー指摘で統一）。以前は連続追加だけ RX を方位矢じりにしていたが、それは *TX/RX が同一座標に重なっても判別する* ための例外で、複数経路を中継の代わりに使っていた時期のもの。中継経路ウィンドウができて使い方が移り、**根拠が消えた例外だけが不統一として残っていた**。⚠️ 文字ラベル（`TX`/`RX`）を出すのは座標入力モードだけで、他は経路が N 本あるので、全端点に字を出すと地図が埋まる（識別は距離バッジに添える経路 ID・地点名が担う）。
...
- **中継点モード**（モードセレクタの「中継点を追加」）: 中継経路のウィンドウから開き、クリックのたびに地点を 1 つずつ末尾へ足します（TX/RX の交互ピックではない）。地点列を結ぶ折れ線と、**区間ごとの水平距離バッジ**を描きます。⚠️ **この距離は画面のここにしか出ない**。区間表の列は周波数・利得・結果だけで距離を持たず、レポートに出るのは**斜距離**（用語集で別語）。地図は写しであって source of truth ではないので、毎回ウィンドウの地点列から引き直します。`append_waypoint`/`waypoint_markers`（＋置き直しの `update_waypoint`）で連携し、実装は `views/multihop_map.py`。
**置き直しは「選んでから」**。マーカーをクリックするとその点が選択状態になり（琥珀のリング＋状態バーに呼び名）、**次の地図クリックでそこへ移ります**（Esc で取り消し・選択は 1 回で使い切る）。⛔ **ドラッグにはしない**。素のドラッグはパン（地図の移動）で埋まっており、奪うと「地図を掴もうとしたら点が動いていた」という*入力が黙って書き換わる*誤操作を作る。規則は 1 つ、**素のクリックは足す・選んでからのクリックは直す**。⚠️ **マーカーだけを入口にしない**。地図を遠くへ送ると唯一の入口が画面の外に出て、置き直しの動線ごと死ぬ。座標入力・連続追加モードでは**地図の右クリック**（「ここに TX を置く」「ここに RX を置く」）が、マーカーの位置に依存しない入口になる（役割を名指しするので「黙って書き換えない」も保てる）。⚠️ tkintermapview の既定の右クリックメニュー（英語・座標コピー）は**張り替えて置き換えている**。

地図がウィンドウへ渡すのは**写しの並びでの位置**（`existing_paths()` / `waypoint_markers()` の何番目か）で、ウィンドウの行番号ではありません（読めない座標の行は写しに出ない）。ウィンドウの側は同じ規則で位置を解き、**書き戻す前に名前（経路 ID・地点名）を照合**して、食い違えば動かさずに断ります（実行結果を行へ返すときの「その入力の結果か」を確かめる形と同じ）。⚠️ **移動量が地形メッシュ（5m）より小さいときは状態バーでそう言う**。それより細かく詰めても標高は同じ格子から拾われることがあり、画面は動いたのに結果が変わらないことがあるため（標本の置き方は「地形の解像度」の段階で決まる）。

- **キャッシュ管理モード**: パン/ズーム追従でキャッシュ済み領域を最高精度で色分け表示（緑=5m航空レーザ / 黄=5m写真測量 / 水色=10m）。ジェスチャは、ドラッグ=移動 / Ctrl+ドラッグ=DL / Ctrl+Alt+ドラッグ=強制再取得 / Shift+Ctrl+ドラッグ=範囲削除。DL・削除は確認ダイアログあり。実装は `dem_prefetch.prefetch_tiles` ほかの公開 API（⚠️ **3.0 で `dem` から `dem_prefetch` へ移した**）。通常の範囲ダウンロードでは、取得済みで**読めるもの**は再 DL しない（外部サーバー配慮。読めなくなっていたタイルは取り直す。3.0 から。強制再取得は既存分も取り直す）。国土地理院の強制再取得では、取り直した上位の層に欠損が無くなって読まれなくなった 5b・10m のタイルを消す（3.7 から。10m を消すのは子の 5m を 4 枚とも取り直した位置だけで、範囲の端の 10m は範囲外で要ることがあるので残す）。全キャッシュ削除は **設定 > 全キャッシュ削除**。
...
1. **DEMタイル事前取得**: TX/RX 間のバウンディングボックス内の全タイルをディスクキャッシュに保存します（最大 8 並列）。キャッシュ済みタイルはスキップされるため 2 回目以降は即完了します。⚠️ **この工程が働くのは組み込みの国土地理院を選んでいるときだけ**です（`views/launcher.py` の実行経路が、それ以外のソースでは事前取得を飛ばして計算へ進む）。宣言ファイルで足した DEM ソースでは、次の標高取得で要る点だけを取りに行きます。範囲をまとめてキャッシュへ入れる地図のキャッシュ管理の範囲ダウンロードは、3.6 から足した DEM ソースにも使えます（`dem_prefetch.prefetch_tiles` の `_prefetch_generic`）。
2. **標高データ取得**: サンプル点ごとに標高を並列取得します（最大 8 並列）。同じ TX/RX 座標・サンプル数であれば前回のキャッシュを即時使用します。

> **地形データの日付**: キャッシュ済みのタイルは取り直さないため、`report.txt` の `DEM Acquired` 行には、実行日ではなく**標高を読んだタイルをこの PC に保存した日**（キャッシュファイルの更新日時・`dem.tile_acquired_date`）が記録されます（3.3 から。経路上で日付が複数にまたがるときは最古〜最新の範囲、1 点も分からないときは行ごと出ません）。この値の算出はネットワークへ出ません。新しいタイルで計算し直したいときは、キャッシュ管理モードでその範囲を強制再取得してください。
...
**個別（ランチャー）は条件を詰める場、複数経路は確定した条件で成果物を作る場**です。ランチャーが唯一の「真実の源（source of truth）」で、複数経路の各行は **行を追加した時点のランチャー欄を文字列コピーして凍結した確定リンク** になります。
...
- `freq` / `gain_tx` / `gain_rx` を省略した場合は共通設定の値が使われます（**リンク識別属性**として経路ごとに変えられる）。環境区分・降雨強度・回折モデルは共通設定で一括指定します
...
- `meas_dbm` / `meas_method` / `feeder_loss_db` / `env_class` は実測突合せ用の任意列（単一ソースは `batch.PathRow`）。`meas_dbm` を入れた経路が1本でもあれば `core/residuals.py` が層別（環境区分×帯域×距離帯）の残差統計を計算し、複数経路のサマリレポートへ表として載る（`report/residuals.py` が `PathResult` から標本を抽出）。`meas_method` は `spot` / `mean` の想定値。複数経路画面の「スポット測定を統計から除外」チェックボックスで `spot` の行を集計から外せる（`core/residuals.py` の `compute_layered_stats(exclude_spot=True)`）。**記録は測定時にしかできない**ので、利用者向けの手順は[マニュアルの「実測との突合せに使う場合の記録項目」](manual_ja.md#実測との突合せに使う場合の記録項目)を参照
...
- **地形は 1 回だけ取る**: `run_scenario()` は FETCH → CALC（→ RENDER）の順に進み、取得は先頭で 1 回。`_fetch_sync()` は `fetch_elevations_cached()` を通すので、同じ経路で条件を振り直しても DEM の再取得は起きません（これがこの画面の使い方そのもの）。
...
- **進捗は「フェーズ」を宣言する**: `Phase` / `Phases`（`FETCH` `CALC` `RENDER`）に重みを宣言し、レポート生成も `artifacts` としてワーカースレッド内の RENDER フェーズで回します。**完了後に View 側で生成しない**。GUI が固まるうえ、その時間が進捗率に現れないため（過去に 2 度踏んだ欠陥）。
- **検証は DEM 取得より前**: `validate_base()` はベース値を取得前に弾きます（取得後に落ちると待ち時間を捨てさせる）。条件側は `Condition` が自分で検査済み。
- **ウィンドウの値が正典**: ランチャーの値はウィンドウを開いた時点で凍結され、**↻ を押すまで取り込まれません**（見えている値で計算するのが、すべてのウィンドウ共通の流儀）。
...
- **トポロジーの宣言は 1 か所**: `TOPOLOGIES` に鎖と星があり、**実際に走れるのは `SUPPORTED_TOPOLOGIES` だけ**。読む・書く・実行するの 3 層がこの 1 つを参照し、ゲートは `require_runnable()`。集約（`ok` / `worst` / `overall_margin`）は**鎖の意味論**なので、鎖以外では黙って数字を返さず止めます（星の min は意味を持たない）。
...
- **判定の単一ソースは `batch.PathResult.status`**: 計算に失敗した区間だけでなく、**成果物が欠けた区間**もここで OK から外れます。全体判定は「一番余裕の少ない区間」の最小値で、**損失は足し合わせません**。
...
> **有効植生長は、隣り合う標本の間の実際の距離で積みます**（台形則。`np.trapezoid(重み, d)`）。
> 標本は等間隔とは限らないため、「重みの合計 × 一定の間隔」では求めません。両端の標本が
> 受け持つのは半区間ずつです（3.0 で是正）。
...
`dem5a_png` → `dem5b_png` → `dem_png` の順に試みます。上位レイヤーが 404 のとき、または画素がデータ欠損 `(128, 0, 0)` かちょうど 0 m `(0, 0, 0)` のときは次のレイヤーに降格します。範囲の事前取得も、下のレイヤーを取るか・残すかを同じ条件で決めます。通常の事前取得も、10m のタイルが無い位置ではキャッシュ済みの 5m のタイルの中身を読み、同じ条件で判定し直します（3.8 から）。3.7 より前に事前取得した範囲で 0 m の画素の下のレイヤーが欠けていても、これで埋まります。

### キャッシュ戦略

- **タイル事前取得**: シミュレーション実行時に TX/RX のバウンディングボックス内の全タイルを事前にディスクキャッシュへ保存（オフライン利用・複数経路の実行の高速化に寄与）。**組み込みの国土地理院を選んでいるときだけ**で、宣言した DEM ソースでは飛ばす（キャッシュ管理の範囲ダウンロード・強制再取得は 3.6 から全ソース対応）
- **メモリキャッシュ**: プロセス内にタイルを保持（キー: `(layer_id, xtile, ytile)`）
- **ディスクキャッシュ**: `terrain_cache/{layer_id}/{xtile}/{ytile}.png` に保存、セッションをまたいで有効。宣言ファイルで足した DEM ソースは 3.6 から `terrain_cache/external/{source_id}/{定義のハッシュ}/{layer_id}/…` に分けて置きます（`core/dem.py` の `DEM_EXTERNAL_SUBDIR`。組み込みの名前と重ならないようにするため）
- **地形キャッシュ**: 同じ TX/RX 座標・サンプル数なら DEM 再取得をスキップ。キャッシュの削除・強制再取得は無効化の世代番号（`core/dem.py` の `_cache_epoch`）を進め、世代の違う項目は使いません（取得中に世代が進んだ結果も登録しません）
...
| ファイル          | 内容                                                       |
| ----------------- | ---------------------------------------------------------- |
| `scenario.html` | A4 縦 1 枚（比較は差分表・Δ をセル内併記 / スイープは折れ線＋表）|
...
| ファイル | 内容 |
| --- | --- |
| `route.html` | 合成シート（全体判定は min ＋区間別の内訳＋俯瞰地図） |
...
> - **判定（`status`）列が多値化されます。** いま `summary.csv` / `hops.csv` / `scenario.csv` の `status` は **OK**（マージン ≥ 0 dB）/ **NG**（< 0 dB）/ **ERROR**（計算か成果物の生成に失敗して判定できなかった場合。`report/batch.py`・`report/multihop.py` が付ける）の 3 値です。4.0 では、感度計算（植生高・環境区分・回折モデルなどを前提側へ振った再計算の幅。3.4 で導入・`core/sensitivity.py`）を判定へつなぎ、**OK**（悲観側の前提でも成立）/ **NG**（楽観側の前提でも不成立）/ **REVIEW**（前提しだいで裏返るため現地確認を推奨）/ **ERROR**（従来どおり判定できなかった場合。**改名しない**）の 4 値にします（`core/models.py` の `status = "OK" if actual_margin >= 0 else "NG"` を置き換える）。列名・位置と、OK / NG を分けるマージン 0 dB の線は変えません。変わるのは、その線に当てるマージンです（いまは既定の前提のマージン 1 つ。4.0 では悲観側・楽観側の両方で当て、どちらでも成立すれば OK・どちらでも不成立なら NG・割れたら REVIEW）。新しい値と条件は 4.0 の CHANGELOG で示します。
> - **末尾に列が 2 つ増えます。** 計算プロファイル ID（既定の計算方式か、旧プロジェクトを開いたときに当時の方式を再現する互換モードかを区別する識別子）と、入力設定のハッシュです。既存の列は動きません。
> - **画面・帳票の「ライスKファクター」（`current_k` は `initial_k − diff_loss / 3`、表示専用）の扱いを見直します。** 根拠となる係数「3」の出典が無く、3.4/3.5/3.6/3.7 で効き方を実測したうえで、4.0 で据え置き・算出方法の変更・欄の廃止のいずれにするかを決めます。**今は何も変わっていません。**
> - **ソースから実行する場合の対応 Python の下限が 3.11 → 3.12 に上がります。** 配布 exe を使っている場合は影響ありません。あわせて `numpy` を 2.5.x 系へ更新します（3.4/3.5/3.6/3.7 では 2.4.4 のまま据え置いています）。
...
| `samples` | 点 | その回線で地形を刻んだ点数。**解像度の段階と経路から行ごとに決まる**ので、同じ CSV でも行によって違います。⚠️ **標本は等間隔ではないため**（「高」「中」は DEM 画素の縁ごとに置きます）、**個々の標本の間隔はこの点数からは求められない**（平均的な間隔だけは `horiz_m ÷ (samples − 1)` で近似できる）。効いている刻みは帳票の「結果の取扱に関する補足」に出る 1 画素の寸法です |
| `horiz_m` | m | 送受信点間の水平距離（整数）。`slant_m` は標高差を含む斜距離なので、実効間隔を見たいときは `slant_m` でなくこちらを使うこと（`horiz_m ÷ (samples − 1)` も、標本が等間隔でない解像度では近似にしかならない） |
| `dem_fail_pct` | % | DEM 取得が通信の失敗で終わった標本の割合（**頭打ちなし**）。海抜0mの正当な値や、国土地理院に元々データが無い地点（404・海上・日本域外）は含まない |
...
| `dem_fail_pct` | % | DEM 取得が通信の失敗で終わった標本の割合（**頭打ちなし**）。区間ごとに DEM 取得が別なので、区間によって違う |
...
| `diff_method` | — | 回折モデル |
| `f1_depth_x` | ×F1 | F1侵入深さ。障害物が F1 半径の**何倍**まで食い込んでいるか（**頭打ちしません**）。`f1_pct` が 100 のとき、`1.00` なら*ちょうど*完全遮蔽、`2.50` なら F1 半径の 2.5 倍まで突き抜けています |
| `axis` | — | スイープの軸の名前（`freq_mhz` など）。比較では空欄 |
| `dem_fail_pct` | % | DEM 取得が通信の失敗で終わった標本の割合（**頭打ちなし**）。条件探索は地形を1回だけ取得して固定するため、**全行で同じ値**になる |

⚠️ 軸に `freq_mhz` / `h_tx` / `h_rx` / `veg_h` を選ぶと、`axis_value` とは別に、その条件で実際に使った値を持つ同名の固定列（例: `freq_mhz`）も並ぶ。両者は意味が違い、`axis_value` は軸として振った値、固定列は**その行で実際に使われた全パラメータの値**。
...
| `elev_source` | — | この標本の標高の出所。`dem_sources.resolve()` が返した `source_id`。予約値は `gsi_dem`（組み込みの国土地理院 DEM）と `unavailable`（通信の失敗で取れなかった）の 2 つ。宣言ファイルで足した外部ソースの標本にはその `source_id` が入る（値域は閉じていない） |
...
**含めないもの**: 計算結果（`results/` 側の仕事）、app 設定（テーマ・言語・プロキシ・座標表示形式）、ウィンドウの位置/大きさ/開閉状態。app 設定を含めないのは、他人のプロジェクトを開いた瞬間に言語やプロキシが変わるのを防ぐためで、読み書きの両側で `config.select_sim` を通します。

### 互換性の規則

- `schema_version` は**ドキュメント全体で整数 1 つ**（セクションごとの版は持ちません）。
- **未知キーは無視／欠損キーは既定値／自分より新しい版は拒否**（`config.load_config` と同じ流儀）。
- `app_version` / `saved_at` は刻印であって**読込の判定には使いません**。
- **セクションが無い（キーが無い）ことは「そのウィンドウの情報を持たない」**であって「空のウィンドウ」ではありません。したがって読込側は触らず、保存側は前回値を持ち越します（複数経路のウィンドウを閉じただけで行が消えたファイルを書かないため）。
- 数値として読めない値（NaN / Inf）は書きません。該当するセクションは保存せず、UI がその旨を知らせます（規格外の JSON を作らない）。

### 読込の流儀

読込時は**ウィンドウを閉じません**（2.7 で変更）。開いているウィンドウの上に `views.dialogs.show_notice` のバーを出し、**押されたときだけ** `replace_rows` / `apply_project_spec` / `apply_project_path` で差し替えます。

⚠️ **既存の `↻ ランチャーから更新` には載せていません**。あちらは凍結バー（共通設定・案件情報）だけを触り、利用者が入れた行・地点には手を出さないボタンで、そこへ差し替えを足すと「共通設定を更新するつもりで押したら行が消えた」という事故になります。

⚠️ **セクションが `None` のファイルではバーを出しません。** `None` は「そのウィンドウの情報を持たない」で、空にする指示ではないためです（出すと「取り込む」が全消しとして働く）。取り込まないままウィンドウを閉じた場合は、閉じる処理が今までどおりそのウィンドウのセクションを持ち越すので、**画面に見えていた内容が残ります**（凍結方式と同じ向き）。
...
```powershell
# 全件。CI と同じくカバレッジゲートも掛かる（手で全体の緑を確かめるとき）
...
- **カバレッジゲートは全件のときだけ**掛かります。部分実行に掛けると、回していない層が 0% で数えられて必ず割れます。
- 出力は**検査ごとに 1 行＋落ちた検査の抜粋**です。全文は `--full-output`。

### 動作確認用の設定ファイル（`qa_fixtures/`）

利用者拡張の宣言ファイル（`dem_sources.toml` / `tile_sources.toml`）と、利用者が追加する表示言語のファイル（`lang\` の下）は、ポータブル配置では exe の隣、つまり `dist\RadioSimPro\` から読みます。ここはビルドのたびに作り直されるので、手で置いた確認用の設定は毎回消えます。そこで **正本を `qa_fixtures/` に置き、ビルドの後に配る**ようにしてあります。
...
- 配置先は**製品のコードに聞きます**（`core.config.USER_DEM_SOURCES_FILE`）。ここに置き場を書き写すと、製品側が動いた日に黙ってずれます。言語ファイル（`qa_fixtures/lang/qa_fr.json`）は配置先の直下ではなく `lang\` の下（`core.config.USER_LANG_DIR` と同じ位置）に置きます。
...
| `test_scenario.py`       | 条件探索（DEM 取得 1 回 + N 条件・フェーズの進捗・上書きの非破壊・A4 シート/CSV 出力）|
| `test_multihop.py`       | 中継経路（waypoint→ホップ導出・中継点の高さ共有・損失を連結しない・全体判定 min・hops.csv／合成シート）|
| `test_project.py`        | プロジェクトファイル（`.rsproj` の往復・app 設定を取り込まない・セクションの欠損は「持たない」・新しい schema の拒否・破損の扱い）|
...
| `test_ground_reflection.py` | 地面反射（2波干渉）の振幅包絡線。反射点が両端から十分離れているかの判定・既存の計算経路を変えないことの回帰（3.4 ステージ4） |
| `test_sensitivity.py`    | 摂動再計算エンジン。各軸が baseline を挟むこと・入力していない軸を出さないこと・回折損と植生減衰の合成規則差し替え（和⇄大きいほう）・標高を経路全体へ一律に動かしても余裕度が動かないこと・多ホップの argmin 入れ替わり（3.4 ステージ4） |
| `test_residuals.py`      | 実測との残差計算。帯域/距離帯の境目・給電線損失の補正方向・層（環境区分×帯域×距離帯）ごとの中央値/IQR/件数・スポット測定の除外（3.4 ステージ5） |
| `test_report_residuals.py` | `PathResult` → 残差標本の抽出。実測値の無い行／計算失敗行を除外すること（3.4 ステージ5） |
| `test_report_sensitivity.py` | 感度表・残差表の帳票配線。地面反射の disclosure ⇔ 感度表の切り替え条件・残差表は標本 0 件で非表示・DEM 標高の軸と多ホップの argmin 注記を出さないこと（3.4 ステージ6） |
...
| `test_update_check.py`   | 更新の確認（`core/update_check.py`）。正式版の利用者には正式だけ・RC/a/b の利用者には RC も知らせること・下書きと読めないタグとリポジトリ外の URL を捨てること・回数の上限／届かない／読めないの見分け・配布形ごとの入れ方の文。起動時の確認＝既定オフ・1 日 1 回まで（試みた日を打つ前に記録）・最新と失敗は黙る・待つ間に手動で押すと答えを出す。プレリリースも知らせる＝触るまでは版で決まり・選んだらそちらが勝ち・手動と起動時の両方の問い合わせへ届く。DEM 取得失敗の文を 3 キーに割っても字が変わらないこと |
...
| `test_title_bar.py`      | タイトルバー・ウィンドウ枠の申告（装飾側 HWND の選び方・ダーク/ライトの値・旧属性番号への退避・**ウィンドウがマップされてから申告しているか**。生成直後は送り先が無く空振りする） |
...
| `test_i18n_key_duplication.py` | i18n キーのゲート（**画面に出る同じ字を 2 つのキーで持たない**。片方だけ直すと画面に 2 通りの語が出るため。成果物であるレポート HTML・グラフ画像の語は対象外で、画面と成果物の名前を揃えるのは出力の版の仕事） |
| `test_i18n_external.py` | **利用者が足す言語**（`lang/<コード>.json`）のゲート。**採用しない側がコア**で、差し込み（`{n}` など）が英語と違うキー・英語に無いキー・成果物の語（`html_*` / `pl_*`）・文字列でない値は採用せず、同梱の `ja` / `en` は上書きさせない。未訳キーが英語へ落ちること、1 本が壊れていても他の言語が読めることも見る。**「弾かなければ実際に `KeyError` になる」ことを毎回実演する 1 本**を含む（この検査が要る理由そのものが消えたら、その 1 本が落ちて教える） |
| `test_i18n_no_hardcoded_ui_text.py` | i18n 迂回のゲート（**画面に出る自然言語を、i18n を通さずに書かない**。`views/` が字を出す 4 つの口である `text=` / `label=` / `.title()` / `dialogs.*()` に渡るリテラルを見て、英単語や仮名漢字が直書きされていれば赤。判定の 3 値・単位・記号・数値書式は両言語で同じなので対象外） |
...
| `test_dev_check.py`      | 検証ランナー自身のゲート（`buildtools/dev_check.py`）。**手書きの対応表が腐って黙って何も足さなくなる**のを検出し、範囲を絞っても静的検査のゲートが必ず足されること・カバレッジゲートが全件のときだけ掛かること・出力が要約に収まることを固定 |
...
| `test_autorun_tool.py` | 無人の駆動スクリプト（`tools/autorun/run.ps1`）。作業票 1 枚ごとに `claude -p` を起こす道具について書く主張＝入力文はファイルから読み差し込みは 4 つだけ・push は子プロセスの送り先を差し替えて塞ぐ・許可を聞かずフックも飛ばさない・票が足りなければ走り出す前に止まる。`-DryRun` の計画だけを見る（git にも claude にも触らない）。⚠️ **`pwsh` が無い環境では skip** |
...
- DEM の水平解像度（組み込みの国土地理院で 5〜10m。宣言した DEM ソースはもっと粗いことがあります）が伝搬計算精度の上限です。建物単体の遮蔽は考慮されません
...
- **起動中に Windows の表示スケールを変えると、ランチャーをドラッグしている間だけ大きさが漂います**（再起動で直ります）。⚠️ **原因は Tk 8.6 側で、アプリからは回避できないことを実測で確認済み**です（10 通り以上試して不発）。**ソースから実行しても同じ**なので、この症状を見ても自分の変更を疑う必要はありません。Tk 9.0 では起きないことも実測済みですが、乗り換えは土台の Python の対応待ちです。症状の詳細は [manual_ja.md](manual_ja.md) の「操作」を参照してください
- **表示スケールの違うモニタ間でウィンドウを移すと、メニューバーだけが元の倍率のまま残ります**（Windows が描く HMENU のため）。表示だけの問題です
- **表示スケールを変えたあとサインインし直すまで、ドロップダウンのチェック（✓）が太く大きく描かれて文字に重なり、カスケードの矢印（▸）が消えます**（サインインし直せば直ります）。Tk の `win/tkWinMenu.c` がこの 2 つを OEM ビットマップ（`OBM_CHECK`／`OBM_MNARROW`）を `LoadBitmapW` で読んで描いており、その大きさはサインイン時の表示スケール（システム DPI）で決まる一方、字は今のモニタの DPI に合わせて当てているためです。**Tk 8.6 と 9.0 の両方が同じ描き方**なので Tk 9 へ移っても直りません。症状は開発機では、サインインし直していない状態で表示スケールを変えれば再現できます
...
**報告の際は、ヘルプメニューの「診断パッケージを保存...」（`core/diagnostics.py`）で作成した ZIP を添付してください。** 版・設定・直近ログ・キャッシュ統計・環境情報・保存先パスがまとまっており、座標や資格情報、パス中のユーザー名は自動で伏字化されます（→ [ヘルプ](#ヘルプ)）。結果ファイルは既定でチェックが入らないため、案件情報を含めるかどうかはご自身で選べます。
