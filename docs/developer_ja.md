...
- **署名**: `RADIOSIM_SIGNTOOL` が定義済みなら、exe 本体と同じ 1 か所の呼び出しでインストーラにも署名します（未定義なら未署名のまま）。現在の配布物は未署名です。⚠️ **「未署名では導入がブロックされる」状況は 2026-09-07 に実際に確認しました**＝Windows 11 の Smart App Control（クリーンインストールした環境でのみ有効になり得る・評価を経て自動的にオン／オフが決まる）が、インストーラが `%TEMP%` へ展開する本体を実行拒否します（上のトラブルシューティング表）。**証明書の取得は検討中で、まだ結論は出ていません**＝自己署名では通らず（Smart App Control は信頼された署名者か評価を要求します）、無償の OV 系サービスで通るかも未検証のためです。それまでの案内はポータブル版 ZIP です。
...
| インストーラが「エラー 4551: アプリケーション制御ポリシーによってこのファイルがブロックされました」で中止（配布先） | **SmartScreen ではなく実行拒否＝続行不可**。Windows 11 の Smart App Control（クリーンインストールした環境でのみ有効になり得る・評価を経て自動的にオン／オフが決まる）または組織のアプリケーション制御が、未署名の `setup.tmp`（Inno Setup が `%TEMP%` へ展開する本体）を弾いている。**MOTW ではないので「ブロックの解除」も効かない。** 当面の回避はポータブル版 ZIP の案内（恒久解は署名）。詳細は `docs/manual_ja.md` の「インストーラが Windows に止められたとき」 |
