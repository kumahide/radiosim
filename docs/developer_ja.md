...
- `meas_dbm` / `meas_method` / `feeder_loss_db` / `env_class` は実測突合せ用の任意列（単一ソース＝`batch.PathRow`）。`meas_dbm` を入れた経路が1本でもあれば `core/residuals.py` が層別（環境区分×帯域×距離帯）の残差統計を計算し、複数経路のサマリレポートへ表として載る（`report/residuals.py` が `PathResult` から標本を抽出）。`meas_method` は `spot` / `mean` の想定値（`spot` は統計から除外可）。**記録は測定時にしかできない**ので、利用者向けの手順は[マニュアルの「実測との突合せに使う場合の記録項目」](manual_ja.md#実測との突合せに使う場合の記録項目)を参照
...
> - **判定（`status`）列が多値化されます。** いま `summary.csv` / `hops.csv` / `scenario.csv` の `status` は **OK**（マージン ≥ 0 dB）/ **NG**（< 0 dB）の 2 値です。4.0 では、感度計算（植生高・環境区分・回折モデルなどを前提側へ振った再計算の幅。3.4 で導入・`core/sensitivity.py`）を判定へつなぎ、**OK**（悲観側の前提でも成立）/ **NG**（楽観側の前提でも不成立）/ **REVIEW**（前提しだいで裏返る＝現地確認を推奨）/ **ERR**（計算不能・データ不足）の 4 値にします（`core/models.py` の `status = "OK" if actual_margin >= 0 else "NG"` を置き換える）。列名・位置・OK / NG それぞれの判定基準は変えません。新しい値と条件は 4.0 の CHANGELOG で示します。
