...
`dem5a_png` → `dem5b_png` → `dem_png` の順に試みます。上位レイヤーが 404 のとき、または画素がデータ欠損 `(128, 0, 0)` かちょうど 0 m `(0, 0, 0)` のときは次のレイヤーに降格します。範囲の事前取得も、下のレイヤーを取るか・残すかを同じ条件で決めます。ただし、通常の事前取得はキャッシュ済みのタイルの中身を判定し直しません。3.7 より前に事前取得した範囲では 0 m の画素の下のレイヤーが欠けたままのことがあり、キャッシュ管理の強制再取得で直ります。
