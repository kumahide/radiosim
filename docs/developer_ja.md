...
| `test_tracer_monitor.py` | 実測補助（`apps/tracer`）の、受信を眺めるだけで何も書かない `monitor`。欠けを電波と USB に分けること（`seq` の飛びから `sample_seq` の飛びを引く）・再起動で番号が戻ったところを欠けに数えないこと・設定は最初と変わったときだけ知らせ、出力は TX の設定にだけ出すこと・受信の無い区切りはそう表示し、CRC 不一致を出すこと |
