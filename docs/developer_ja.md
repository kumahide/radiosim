...
│
├── apps/
│   └── field/           # 実測補助（別アプリ・開発中。本体の配布物には入らない）
│       ├── session.py    # 測定セッション（1 回の測定＝1 フォルダ。ヘッダ JSON ＋追記のみの CSV）
│       ├── aggregate.py  # 受信サンプルをウィンドウごとにまとめ、打ち切りを判定してバッチ CSV へ書き出す
│       ├── recorder.py   # 測りながら書く（UART のバイト列と作業者の操作をセッションへ追記・生ログからの作り直し）
│       ├── calib.py      # 個体ごとの校正ファイル（机上試験が書き、記録が個体 ID で読んでセッションへ写す）
│       ├── bench.py      # ステージ0 の机上試験（2 台の役割をコマンドで入れ替えて両方向を測り、生ログと段を残す）と、人の操作が要らない動作確認
│       ├── bench_analysis.py # 机上試験の判定（生ログから数え直す・線形性・ディザ・受信率・校正ファイル）
│       ├── cli.py        # コマンドライン（雛形の書き出し・記録・バッチ CSV への書き出し・動作確認・机上試験・連続送信）
│       ├── mavlink/      # 受信サンプル・設定のメッセージ定義（ファームと PC 側の両方がここから生成する）
│       │   ├── dialect.py # 定義の XML を読み、フィールドの並び・CRC を組み立てる
│       │   ├── reader.py  # UART のログをフレームへ分解し、受信サンプルへ変える
│       │   └── gen_c.py   # 定義の XML からファーム用の C ヘッダ（見本のフレームつき）を作る
│       └── firmware/     # 送受信機のファーム（XIAO ESP32C6・ESP-IDF。手順は README.md）
...
> - **判定（`status`）列が多値化されます。** いま `summary.csv` / `hops.csv` / `scenario.csv` の `status` は **OK**（マージン ≥ 0 dB）/ **NG**（< 0 dB）/ **ERROR**（計算か成果物の生成に失敗して判定できなかった場合。`report/batch.py`・`report/multihop.py` が付ける）の 3 値です。4.0 では、感度計算（植生高・環境区分・回折モデルなどを前提側へ振った再計算の幅。3.4 で導入・`core/sensitivity.py`）を判定へつなぎ、**OK**（悲観側の前提でも成立）/ **NG**（楽観側の前提でも不成立）/ **REVIEW**（前提しだいで裏返るため現地確認を推奨）/ **ERROR**（従来どおり判定できなかった場合。**改名しない**）の 4 値にします（`core/models.py` の `status = "OK" if actual_margin >= 0 else "NG"` を置き換える）。列名・位置・OK / NG それぞれの判定基準は変えません。新しい値と条件は 4.0 の CHANGELOG で示します。
> - **末尾に列が 2 つ増えます。** 計算プロファイル ID（既定の計算方式か、旧プロジェクトを開いたときに当時の方式を再現する互換モードかを区別する識別子）と、入力設定のハッシュです。既存の列は動きません。
> - **画面・帳票の「ライスKファクター」（`current_k` は `initial_k − diff_loss / 3`、表示専用）の扱いを見直します。** 根拠となる係数「3」の出典が無く、3.4/3.5 で効き方を実測したうえで、4.0 で据え置き・算出方法の変更・欄の廃止のいずれにするかを決めます。**今は何も変わっていません。**
> - **ソースから実行する場合の対応 Python の下限が 3.11 → 3.12 に上がります。** 配布 exe を使っている場合は影響ありません。あわせて `numpy` を 2.5.x 系へ更新します（3.4/3.5 では 2.4.4 のまま据え置いています）。
...
依存の宣言は **2 ファイルに分かれています**（別アプリの実測補助の分は下の表の 3 行目）。
...
| `requirements-field.txt` | 実測補助（`apps/field`）がシリアルポートを開くための pyserial。テストには要らない（CI も入れない） | **入らない**（本体とは別のアプリ） |
...
```powershell
# コミット前（本番）は全件。CI と同じくカバレッジゲートも掛かる
...
| `test_field_session.py` | 実測補助（`apps/field`）の測定セッション。ヘッダの往復・サンプルの追記のみ・ヘッダの原子的な書き込み・位置は定数と時系列の両方を許すこと・時間平均と空間平均が別の欄に在ること・生値を換算値で上書きしないこと・給電線損を換算に混ぜないこと・CLAS の高さは Fix 解のときだけ通すこと・語彙の外の値と空の測定構成 ID を保存しないこと |
| `test_field_aggregate.py` | 実測補助（`apps/field`）のウィンドウの集計と打ち切り。ウィンドウの刻みはシーケンス番号・一部だけ届いたウィンドウは値を出さないこと・1 つも届かなかったウィンドウも行として残ること・置き場所の区切りは操作の記録から決めること（動かす直前と終了の直前に途絶えた分が消えない・1 つも受からなかった置き場所もウィンドウになる・移動中の受信はウィンドウに入れない）・設定と置き場所を跨いで平均しないこと・平均は dB 領域であること・TX 側の給電線損が二重に効かないこと・バッチ CSV の原子的な書き込み |
| `test_field_recorder.py` | 実測補助（`apps/field`）の記録と CLI。読み出しの区切りに跨ったフレームを落とさないこと（どう区切っても同じメッセージ・同じエラー数）・設定が届くまでセッションを作らず、それまでの生のバイト列も残すこと・生ログから作り直したサンプルがライブで書いたものと一致すること・雛形の未記入と別個体の校正で記録を始めないこと・設定が途中で変わったら終えること・別の組のサンプルを書かないこと・終了の記録が無いセッションを黙って書き出さないこと |
| `test_field_mavlink.py` | 実測補助（`apps/field`）の MAVLink の読み取り。ワイヤ上の並びが型のサイズ順であること・CRC_EXTRA が定義から作られること・末尾を切り詰めた payload をゼロで埋め戻すこと・CRC を通らないフレームからサンプルを作らないこと・知らない ID を丸ごと読み飛ばして誤り数を水増ししないこと・末尾の切れを化けと区別すること・フレームのリンク番号を測定のシーケンス番号に使わないこと・UART で落ちた分が打ち切りとして現れること |
| `test_field_firmware.py` | 実測補助（`apps/field`）のファームと PC 側の境目。生成した C ヘッダが定義の XML と一致すること・ヘッダに埋め込んだ見本のフレームを PC 側で読むと見本の値に戻ること（ファームは起動時に同じフレームを自分で組めるかを確かめる）・見本が並びや符号の取り違えを隠さない値であること・自己検査より前に何も送らないこと・設定を 1 秒ごとに再送し、PC 側がその再送で記録を止めないこと・本当に変わった設定では止めること・USB にログを流さないこと |
| `test_field_bench.py` | 実測補助（`apps/field`）のステージ0 の机上試験と校正ファイル。2 台と空中の経路を模擬し、役割はコマンドで入れ替わってアッテネータ 1 段につき頼むのは 1 回で両方向が測られること・RX が中継する相手の設定と自分の設定を取り違えないこと・公称の入力では dBm を名乗る校正ファイルを作らないこと・パワー計の実測を渡すと測り直さずにRX の絶対値と TX の出力の表（出どころつき）が出ること・漏れ・ディザの欠如・飽和を見逃さないこと・PC までの間の欠けを電波の欠けと混ぜないこと・校正ファイルが個体 ID で動いている設定の行を写すこと・連続送信が測定と同じ送信経路を使い、報告の書式を PC 側が同じに読むこと・動作確認が人に何も頼まずに両方向を測り、USB で落ちたサンプルを電波の欠けと分けて不合格にすること（番号が戻ったところは数えない）・ファームの版をリポジトリのコミットと照合し、機器から届いた版は 16 進の形のときだけ git に渡すこと・動作確認のフォルダを机上試験として判定しないこと |
| `test_field_monitor.py` | 実測補助（`apps/field`）の、受信を眺めるだけで何も書かない `monitor`。欠けを電波と USB に分けること（`seq` の飛びから `sample_seq` の飛びを引く）・再起動で番号が戻ったところを欠けに数えないこと・設定は最初と変わったときだけ知らせ、出力は TX の設定にだけ出すこと・受信の無い区切りはそう表示し、CRC 不一致を出すこと。あわせて設定を表示・変更する `settings`。範囲の外の値を送る前に止めること・役割を最後に送ること・中継された他の TX の設定ではなく自分の設定を読むこと・変えたと答えたのに値が変わっていない機器を見逃さないこと・機器の断りをそのまま出すこと |
...
| `test_pre_commit_gate.py` | コミット前ゲート（`tools/qa-hook/pre-commit-gate.mjs`）の「島」の判定。変更がすべて 1 つの島（例: Field）に収まるときだけ島のテストに絞り、島の外・リネーム元・空の変更ならフルに戻ること、リポジトリ全体を走査するテストが島のテストから漏れていないことを検証。⚠️ **`node` が無い環境では skip** |
