...
│       ├── bench.py      # 段0 の机上試験（2 台の役割をコマンドで入れ替えて両方向を測り、生ログと段を残す）
│       ├── bench_analysis.py # 机上試験の判定（生ログから数え直す・線形性・ディザ・受信率・校正ファイル）
│       ├── cli.py        # コマンドライン（雛形の書き出し・記録・バッチ CSV への書き出し・机上試験・連続送信）
