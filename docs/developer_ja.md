...
欄の整形が走るのは**起動時・表記の切替時・設定/プロジェクトの読込時・入力の確定（Enter / focus 離脱）**。⚠️ **入力できる座標欄 3 面はすべて確定でも整形する**（ランチャー `views/launcher.py`・複数経路の表 `views/batch_table.py`・中継経路 `views/multihop.py`＝2.7 で揃えた）＝**整形されること自体が parse 成功の返事**で、読めなければ原文が残る。⚠️ 条件探索（`views/scenario.py`）の座標は**読み取り専用**（ランチャーの凍結スナップショット）なので対象外＝`_update_path_label` の再描画でしか整形されない。
...
ランチャーの**「地図」ボタン**（`views/map_window.py`）で、国土地理院の淡色地図を使う補助ウィンドウを開きます。**地図はアプリ唯一のインスタンス（ランチャー所有）**で、上部のモードセレクタで 4 モードを切り替えます。コアのシミュレーションは地図なしで完結し、地図は便利機能です。開いたとき設定中 TX/RX の経路長に合わせて自動ズーム・センタリングします。⚠️ **複数経路のウィンドウからも地図を開けます**（「地図から選択」＝連続追加モードで開く。2.7 で追加＝それまでは地図の側からしか連続追加を始められなかった）。
...
> - **判定（`status`）列が多値化されます。** いま `summary.csv` / `hops.csv` / `scenario.csv` の `status` は **OK**（マージン ≥ 0 dB）/ **NG**（< 0 dB）/ **ERROR**（計算か成果物の生成に失敗して判定できなかった＝`report/batch.py`・`report/multihop.py` が付ける）の 3 値です。4.0 では、感度計算（植生高・環境区分・回折モデルなどを前提側へ振った再計算の幅。3.4 で導入・`core/sensitivity.py`）を判定へつなぎ、**OK**（悲観側の前提でも成立）/ **NG**（楽観側の前提でも不成立）/ **REVIEW**（前提しだいで裏返る＝現地確認を推奨）/ **ERROR**（従来どおり＝判定できなかった。**改名しない**）の 4 値にします（`core/models.py` の `status = "OK" if actual_margin >= 0 else "NG"` を置き換える）。列名・位置・OK / NG それぞれの判定基準は変えません。新しい値と条件は 4.0 の CHANGELOG で示します。
> - **末尾に列が 2 つ増えます。** 計算プロファイル ID（既定の計算方式か、旧プロジェクトを開いたときに当時の方式を再現する互換モードかを区別する識別子）と、入力設定のハッシュです。既存の列は動きません。
> - **画面・帳票の「ライスKファクター」（`current_k`＝`initial_k − diff_loss / 3`、表示専用）の扱いを見直します。** 根拠となる係数「3」の出典が無く、3.4/3.5 で効き方を実測したうえで、4.0 で据え置き・算出方法の変更・欄の廃止のいずれにするかを決めます。**今は何も変わっていません。**
...
| `samples` | 点 | その回線で地形を刻んだ点数。**解像度の段階と経路から行ごとに決まる**ので、同じ CSV でも行によって違います。⚠️ **標本は等間隔ではありません**（「高」「中」は DEM 画素の縁ごとに置きます）＝**個々の標本の間隔はこの点数からは求められない**（平均的な間隔だけは `horiz_m ÷ (samples − 1)` で近似できる）。効いている刻みは帳票の「結果の取扱に関する補足」に出る 1 画素の寸法です |
...
⚠️ 軸に `freq_mhz` / `h_tx` / `h_rx` / `veg_h` を選ぶと、`axis_value` とは別に、その条件で実際に使った値を持つ同名の固定列（例: `freq_mhz`）も並ぶ。両者は意味が違う＝`axis_value` は軸として振った値、固定列は**その行で実際に使われた全パラメータの値**。
...
| `test_qa_gate_cache.py`  | QA ゲート（`tools/qa-hook/pytest-cache.mjs`）の再実行抑止キャッシュ。鍵が「作業ツリーの中身」で動くこと＝内容が変われば必ず走り、変わらなければ走らないことを検証。⚠️ **`node` が無い環境では skip**（対象そのものは 2026-08-12 以降 git 管理下にある） |
