...
| `requirements-tracer.txt` | 実測補助（`apps/tracer`）がシリアルポートを開くための pyserial。テストには要らない（CI も入れない） | **入らない**（本体とは別のアプリ） |
...
| `test_tracer_aggregate.py` | 実測補助（`apps/tracer`）の窓の集計と打ち切り。窓の刻みはシーケンス番号・一部だけ届いた窓は値を出さないこと・1 つも届かなかった窓も行として残ること・置き場所の区切りは操作の記録から決めること（動かす直前と終了の直前に途絶えた分が消えない・1 つも受からなかった置き場所も窓になる・移動中の受信は窓に入れない）・設定と置き場所を跨いで平均しないこと・平均は dB 領域であること・TX 側の給電線損が二重に効かないこと・バッチ CSV の原子的な書き込み |
| `test_tracer_recorder.py` | 実測補助（`apps/tracer`）の記録と CLI。読み出しの区切りに跨ったフレームを落とさないこと（どう区切っても同じメッセージ・同じエラー数）・設定が届くまでセッションを作らず、それまでの生のバイト列も残すこと・生ログから作り直したサンプルがライブで書いたものと一致すること・雛形の未記入と別個体の校正で記録を始めないこと・設定が途中で変わったら終えること・別の組のサンプルを書かないこと・終了の記録が無いセッションを黙って書き出さないこと |
| `test_tracer_mavlink.py` | 実測補助（`apps/tracer`）の MAVLink の読み取り。ワイヤ上の並びが型のサイズ順であること・CRC_EXTRA が定義から作られること・末尾を切り詰めた payload をゼロで埋め戻すこと・CRC を通らないフレームからサンプルを作らないこと・知らない ID を丸ごと読み飛ばして誤り数を水増ししないこと・末尾の切れを化けと区別すること・フレームのリンク番号を測定のシーケンス番号に使わないこと・UART で落ちた分が打ち切りとして現れること |
| `test_tracer_firmware.py` | 実測補助（`apps/tracer`）のファームと PC 側の境目。生成した C ヘッダが定義の XML と一致すること・ヘッダに埋め込んだ見本のフレームを PC 側で読むと見本の値に戻ること（ファームは起動時に同じフレームを自分で組めるかを確かめる）・見本が並びや符号の取り違えを隠さない値であること・自己検査より前に何も送らないこと・設定を 1 秒ごとに再送し、PC 側がその再送で記録を止めないこと・本当に変わった設定では止めること・USB にログを流さないこと |
| `test_tracer_bench.py` | 実測補助（`apps/tracer`）の段0 の机上試験と校正ファイル。2 台と空中の経路を模擬し、役割はコマンドで入れ替わってアッテネータ 1 段につき頼むのは 1 回で両方向が測られること・RX が中継する相手の設定と自分の設定を取り違えないこと・公称の入力では dBm を名乗る校正ファイルを作らないこと・パワー計の実測を渡すと測り直さずにRX の絶対値と TX の出力の表（出どころつき）が出ること・漏れ・ディザの欠如・飽和を見逃さないこと・PC までの間の欠けを電波の欠けと混ぜないこと・校正ファイルが個体 ID で動いている設定の行を写すこと・連続送信が測定と同じ送信経路を使い、報告の書式を PC 側が同じに読むこと・動作確認が人に何も頼まずに両方向を測り、USB で落ちたサンプルを電波の欠けと分けて不合格にすること（番号が戻ったところは数えない）・ファームの版をリポジトリのコミットと照合し、機器から届いた版は 16 進の形のときだけ git に渡すこと・動作確認のフォルダを机上試験として判定しないこと |
| `test_tracer_monitor.py` | 実測補助（`apps/tracer`）の、受信を眺めるだけで何も書かない `monitor`。欠けを電波と USB に分けること（`seq` の飛びから `sample_seq` の飛びを引く）・再起動で番号が戻ったところを欠けに数えないこと・設定は最初と変わったときだけ知らせ、出力は TX の設定にだけ出すこと・受信の無い区切りはそう表示し、CRC 不一致を出すこと。あわせて設定を表示・変更する `settings`。範囲の外の値を送る前に止めること・役割を最後に送ること・中継された他の TX の設定ではなく自分の設定を読むこと・変えたと答えたのに値が変わっていない機器を見逃さないこと・機器の断りをそのまま出すこと |
