...
> - **画面・帳票の「ライスKファクター」（`current_k` は `initial_k − diff_loss / 3`、表示専用）の扱いを見直します。** 根拠となる係数「3」の出典が無く、3.4/3.5/3.6 で効き方を実測したうえで、4.0 で据え置き・算出方法の変更・欄の廃止のいずれにするかを決めます。**今は何も変わっていません。**
> - **ソースから実行する場合の対応 Python の下限が 3.11 → 3.12 に上がります。** 配布 exe を使っている場合は影響ありません。あわせて `numpy` を 2.5.x 系へ更新します（3.4/3.5/3.6 では 2.4.4 のまま据え置いています）。
