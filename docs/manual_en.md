...
**Sensitivity range** (heading "Sensitivity to input uncertainty", at the bottom of the "Notes on handling this result" section): shown on single-path, Multiple Paths, relay, and condition-explorer reports. It first states the baseline margin once, then lists one row each — for whichever apply to that link — as a margin range (dB): DEM elevation (±3 m), vegetation height (±30%), environment class (to the adjacent one), diffraction model (single ⇄ Bullington), how diffraction loss and vegetation attenuation are combined (summed ⇄ take-the-larger), the terrain resolution step, and ground reflection (the two-ray interference spread). Inputs that make no measurable difference are grouped into one summary line instead of a row. Relay reports add a row for the **weakest section** (the one with the least margin), plus a note on whether nudging the DEM elevation the same way across every hop changes which section is weakest (and to which section, if so). On Multiple Paths and condition-explorer reports, this range is a **reference for the baseline condition only** (the other conditions are not evaluated).

**Residuals table** (heading "Measured-vs-predicted residuals"): shown on the Multiple Paths summary report. It only appears when at least one path in the batch has `meas_dbm` filled in, and shows the count, median, and spread (IQR) of the residual (predicted minus measured, after adding back the feed-line loss) for each **environment class × band × distance band** layer. Rows where `meas_method` is `spot` (single-point readings) can be excluded from the statistics if needed, since they can be off by ±5-10 dB due to multipath. The table is omitted entirely for batches with no measured values.
