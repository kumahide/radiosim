...
> 📣 **Advance notice (coming in 4.0 — nothing changes in 3.4/3.5/3.6/3.7)**:
...
> - **The on-screen/report "Rice K factor" (the current-value estimate, computed as `initial_k − diff_loss / 3`, display-only) will be reconsidered.** The "3" in that formula has no cited basis. After measuring how much it actually matters in 3.4/3.5/3.6/3.7, 4.0 will decide whether to leave it as is, change how it's computed, or drop the field. **Nothing changes about it right now.**
> - **The minimum supported Python version, for running from source, rises from 3.11 to 3.12 in 4.0.** This has no effect if you use the packaged exe (installer or ZIP). `numpy` will also move to the 2.5.x line at the same time (3.4/3.5/3.6/3.7 keep it pinned at 2.4.4).
