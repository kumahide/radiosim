...
- **Words that appear in artifacts are out of scope.** This covers reports (HTML) and KML, and also **the wording inside the terrain profile and comparison graphs** (axis names, legends, titles, the way units are bracketed). Artifact wording is a contract with whatever reads the files, so it does not move for the screen's convenience. ⚠️ **Those figures show the same wording on screen**: saving one turns it into an artifact as-is, so the on-screen side cannot be translated on its own.
