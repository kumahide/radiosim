...
> 📣 **Advance notice (coming in 4.0 — nothing changes in 3.4)**:
> - **The `status` column becomes multi-valued.** Today, `status` in `summary.csv` / `hops.csv` / `scenario.csv` is a 2-value field: **OK** (margin ≥ 0 dB) / **NG** (< 0 dB). In 4.0, the sensitivity calculation introduced in 3.4 (a recalculation across pessimistic/optimistic assumptions — vegetation height, environment class, diffraction model, and so on) will feed into the verdict, giving four values: **OK** (holds even under pessimistic assumptions) / **NG** (fails even under optimistic assumptions) / **REVIEW** (flips depending on assumptions — a site visit is recommended) / **ERR** (could not be computed — missing data). The column name, position, and the OK/NG thresholds themselves do not change. **If your code branches on `status == "OK"`, we recommend treating anything other than `OK` as "needs review" from 4.0 onward** — collapsing `REVIEW` into NG would drop links that a site visit would actually confirm. The new values and the conditions for each will be spelled out in the 4.0 CHANGELOG.
...
> - **The minimum supported Python version, for running from source, rises from 3.11 to 3.12 in 4.0.** This has no effect if you use the packaged exe (installer or ZIP). `numpy` will also move to the 2.5.x line at the same time (3.4 keeps it pinned at 2.4.4).
> - ⚠️ **This notice follows change policy 2** (published in both the CHANGELOG and this manual). **It isn't yet decided whether 3.4 or 3.5 is "the version before" 4.0**, so this is published early rather than waiting for that to be settled.
