...
- **Always present**: elevations come from a **bare-earth model** (no buildings, no trees); vegetation height is **the single value entered**, applied along the whole path; environment loss is an **empirical figure taken from the area class**; Earth curvature uses the **standard-atmosphere factor K = 1.33, fixed**, so periods when refraction departs from it (sub-refraction, ducting) are not covered; the **Rice K factor shown on screen and in the reports is an empirical estimate** derived from the diffraction loss, not used in the calculation and never compared against measurements; and **the terrain resolution step this result was produced with** (a finer step raises the diffraction loss — up to +407% between the 20 m "low" step and the pixel-edge "high" step).
  - **Ground reflection (two-ray interference)**: on a single-path report, when the specular point sits far enough from both ends for the DEM to estimate a spread, this note is replaced by a **range row** in the sensitivity table below (otherwise the "not modelled" note stays). Multiple Paths, relay, and condition-explorer reports always keep the "not modelled" note, since they cover more than one link at once.
- **Present when they apply**: the `Bullington` model reads low where two or more ridges are well separated, and the spherical-earth term is not included; rain or gaseous attenuation was **out of range and taken as 0 dB** (below 1 GHz); the vegetation coefficient was evaluated **outside its 1–6 GHz definition**; on links **with a vegetation height entered**, that where terrain blocks the path the diffraction loss and the vegetation attenuation are both charged (the total reads high — the safe side). ⚠️ **A link with no rain assumed gets no rain note** — limits of things that were not calculated are not listed, and a link with no vegetation gets no vegetation note.
...
### Sensitivity and residuals tables (from 3.4)
...
**Sensitivity table** (heading "Sensitivity to input uncertainty"): shown on single-path, Multiple Paths, relay, and condition-explorer reports. For whichever apply to that link, it lists one row each — for DEM elevation (±3 m), vegetation height (±30%), environment class (one step), diffraction model (single ⇄ Bullington), how diffraction loss and vegetation attenuation are combined (summed ⇄ take-the-larger), the terrain resolution step, and ground reflection (the two-ray interference spread) — giving the baseline, low, and high margin (dB) for each. Relay reports add a row for the **bottleneck hop** (the one with the least margin), plus a note on whether nudging the DEM elevation the same way across every hop changes which hop is the bottleneck (and to which hop, if so). On Multiple Paths and condition-explorer reports, the sensitivity table is a **reference for the baseline condition only** (the other conditions are not evaluated).
