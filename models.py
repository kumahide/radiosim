...
# {env_type: (base_dB, blocked_coeff, veg_coeff, dist_coeff, diff_coeff, min_dB, max_dB)}
#
#   base_dB      : ベース損失（マルチパス・散乱の最低限）
#   blocked_coeff: Fresnel 遮蔽率 [%] に対する係数
#   veg_coeff    : 植生減衰 [dB] に対する係数
...
ENV_COEFFS: dict[str, tuple] = {
    #           base  blk    veg   dist  diff  min   max
    "urban"   : (10.0, 0.08, 0.20, 1.20, 0.15, 6.0, 30.0),
    "suburban": ( 6.0, 0.05, 0.15, 0.80, 0.10, 3.0, 30.0),
    "rural"   : ( 4.0, 0.03, 0.10, 0.50, 0.08, 2.0, 25.0),
    "los"     : ( 2.0, 0.01, 0.05, 0.30, 0.05, 1.0, 15.0),
}
...
def calculate_propagation(
    ...
    # Fresnel 第1ゾーン半径（ITU-R P.526: 分母は d1+d2）
    f1 = np.sqrt(lam * d1 * d2 / (d1 + d2))
    f1 = np.nan_to_num(f1)

    veg_layer           = elevs + veg_h
    obstruction_surface = np.maximum(elevs, veg_layer)
    ...
    veg_loss = _vegetation_loss(
        elevs, los_vals, f1, freq_mhz, terrain.horiz_dist_km, N
    )
    ...
    # Env Loss（環境区分別推定）
    env_loss  = _env_loss(blocked_ratio, veg_loss, slant_dist_km, diff_loss, env_type)
...
def _nu(h_obs: float, d1_m: float, d2_m: float, lam: float) -> float:
    """
    Fresnel-Kirchhoff パラメータ ν を計算する。

    Args:
        h_obs : 障害物頂点と TX-RX 結線との距離 [m]（正=遮蔽、負=余裕）
        d1_m  : TX ← 障害物 間の水平距離 [m]
        d2_m  : 障害物 → RX 間の水平距離 [m]
        lam   : 波長 [m]
    """
    d1 = max(d1_m, 1.0)
    d2 = max(d2_m, 1.0)
    denom = lam * d1 * d2 / (d1 + d2)
    if denom <= 0:
        return 0.0
    return float(h_obs * math.sqrt(2.0 / denom))
...
def _diffraction_loss(v: float, blocked_ratio: float) -> float:
    """後方互換ラッパー（旧 API を呼ぶ外部コードのため残す）。"""
    return _diffraction_loss_fk(v)
...
def _vegetation_loss(
    ...
    """
    植生層の Fresnel ゾーン侵入深さから減衰量 [dB] を推定する。
    上限 45 dB。
    """
    ...
    fresnel_lower    = los_vals - f1
    penetration      = elevs - fresnel_lower
...
def _env_loss(
    ...
    Args:
        env_type: "urban" | "suburban" | "rural" | "los"
                  ENV_COEFFS で定義された環境区分。未知の値は suburban にフォールバック。
    """
    base, blk_c, veg_c, dist_c, diff_c, min_db, max_db = ENV_COEFFS.get(
