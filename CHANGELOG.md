...
### 修正
- **設定・キャッシュ・結果・ログの保存先が起動方法によって変わる（B-014・2.4RC3）**: 設定ファイル・結果フォルダ・ログ・DEM キャッシュの場所が**カレントディレクトリ基準の相対パス**だったため、ショートカットの作業フォルダやコマンドラインの cwd 次第で**保存先が黙って移っていた**。エラーも警告も出ず、設定が既定値で開き、DEM を取り直し、過去の結果が「結果フォルダを開く」からも見えなくなる（作業フォルダが exe の隣を指す通常のショートカット起動では起きないため、長く残っていた潜在的な不具合）。書き込み先の基準を **exe（スクリプト実行時はソース）の位置**へ固定した。**通常の起動では従来とまったく同じ場所を指す**ので、既存の設定・キャッシュ・結果はそのまま引き継がれる。回帰ガード＝`tests/test_paths.py`（cwd を変えても保存先が動かないこと・従来と同じ場所を指すこと・単一/バッチ/地図の全経路が同じ解決を通ること）。
- **DEM 取得の一時失敗がプロセス終了まで残り標高が誤る（B-010・2.4RC2）**: DEM タイル取得に失敗したタイルを記憶する負キャッシュ（`_failed_tiles`）が、**恒久欠落（日本域外・海上で GSI が返す 404）と一時失敗（タイムアウト・接続断・5xx）を区別せず**両方を永久保持していた。このため通信の一時的な失敗が同一プロセス中そのタイルを汚染し続け、回復後も取得済みタイルを無視して標高が粗レイヤ由来か 0.0 に化けたまま**黙って誤った計算結果**を出しうる（常駐する GUI で顕在化。単一・バッチ・プレビューは同一の標高取得経路を共有するため全フローに波及）。負キャッシュへの登録責務を `get_elevation` から `_fetch_tile` へ移し、**HTTP ステータスで恒久/一時を分離**（404 のときだけ登録・それ以外は登録しない）。戻り値契約は不変のためプリフェッチ／淡色地図／レポート地図は無改修。回帰ガード＝`tests/test_dem.py::TestFailedTileNegativeCache`（一時失敗→回復で標高を取得し直すこと・404 は記憶して再取得を抑止することを検証）。Codex レビューで発見。
- **ダークテーマでツールチップの文字が読めない（B-009・2.4b3）**: 入力欄の説明ツールチップが、背景は OS 固定色・前景はテーマ追従という**別々の出所**から色を取っていたため、ダークでは白文字が淡いグレーの背景に載って判読不能だった（コントラスト 1.06:1）。テーマを一度切り替えると発生する。配色を `views/theme.py` から前景・背景まとめて取るようにした。B-008 の掃き出しで発見（ユーザー報告ではない）。
- **メニューの配色が実際には適用されていなかった（B-008・2.4b3）**: 症状は「ライトテーマでアクティブ行のカスケード ▶ が背景と同化して見えない」。原因は色の取得元で、`ttk.Style().lookup("TFrame", "background")` は sv_ttk（Sun Valley）では**常に空文字を返す**（このテーマは背景色・前景色を設定せず外観を画像で描くため）。空でも例外にならないので、「テーマ色を明示適用する」コードが**黙って何もせず終了**していた。この取得方法を使っていた 4 箇所すべてが同じ状態で、**ダークモードのメニュー ✓（2.4a1 で修正したはずのもの）とバッチ表のキャンバス背景も、実際には効いていなかった**。配色の出所を `views/theme.py` へ単一化し、sv_ttk 自身の色定義から読むようにしたうえで、バッチの行右クリックメニュー（従来テーマ適用の対象外）も含めて載せ替えた。アクティブ時も前景色は変えない方針にしている（`tk.Menu` の ✓ の色は状態別に指定できないため、アクティブだけ別色にすると ✓ か ▶ のどちらかが必ず溶ける）。回帰ガード＝`tests/test_theme.py`＝色が付くことに加え、**前景と背景のコントラスト比が 4.5:1 以上あることを数値で検証**する。
...
- **進捗まわりを3実行フローで対称化（2.4b3・B-006/I-008 の再発防止）**: 実行フローは 単一 / バッチ / 地図タイル取得 の3つ。①phase 境界ログを3フローに揃えた（従来はバッチのみで、単一のグラフ描画も地図の後処理もログ上見えなかった）。②地図のタイル取得も `ProgressPump` 経由に統一（従来はタイルごとに `after(0, …)` を2回呼ぶ、他フローで廃した書き方だった）。③3フローが同じ部品を使うことを `test_all_execution_flows_use_the_progress_pump` で静的に固定。
