"""
apps/field/aggregate.py
========================
受信サンプルを**ウィンドウごとにまとめ**、本体のバッチ CSV へ書き出す（Field phase 1・増分2）。

    samples.csv（生値・1 受信 1 行）＋ events.csv（置き場所の区切り）
      → ウィンドウ（受信率と代表値） → バッチ CSV（1 ウィンドウ 1 行）

**ウィンドウはシーケンス番号で刻む（時刻ではない）。** TX は等間隔で番号付きのパケットを
送るので、ウィンドウの期待パケット数は `集計ウィンドウの長さ ÷ 送信間隔` で決まり、その範囲に
実際に届いた番号を数えれば受信率がそのまま出る。時刻で刻むと、**届かなかった
パケットには時刻が無い**ので分母を作れない（欠けているものは数えられない）。

**打ち切り**（→ [[project-radiosim-field]] §5-4・§6.1-2）
  受信率が刻印のしきい値を下回るウィンドウは、値を出さずに打ち切りとして記録する。
  ⛔ **一部だけ届いたウィンドウの平均を実測として出さない**＝弱いパケットから先に落ちるので、
...
実装を同じ領域に揃える。⚠️ **速いフェージングがウィンドウに入ると、dB 平均は真の平均電力
より低く出る**（負に偏る）。受け入れ試験（§6.1-3 のステージ0）でこの偏りを測る。
...
落ち方は受信レベルと関係ないので、ウィンドウの受信率の**分母から引く**（残りは偏りの無い
部分集合）。⚠️ 電波の欠けと混ざっていて、しかもウィンドウを跨ぐものは、どのウィンドウの分か分からない
＝**どのウィンドウにも振らない**（受信率を高く見せる向きには動かさない）。
...
class Window:
    """1 つの集計ウィンドウ。**打ち切りでも行として残る**（捨てると統計が偏る）。"""

    index: int                  # セッションの中の通し番号（CSV の id になる）
    config_id: int
    spatial_slot: int           # 空間平均の置き場所。ウィンドウはここを跨がない
    seq_start: int              # このウィンドウが覆うシーケンス番号の先頭
...
def window_seq_count(header: SessionHeader) -> int:
    """1 ウィンドウが覆うシーケンス番号の数＝期待パケット数。

    送信間隔は **TX の設定**から取る（受信側の積分ウィンドウではない）。
    """
    ...
    if count < 1:
        raise SessionError(
            f"集計ウィンドウ（{header.provenance.censor_window_s} 秒）が送信間隔"
...
def aggregate(
    ...
    """受信サンプルをウィンドウへまとめる。**サンプルはシーケンス番号の昇順**で渡すこと。

    ウィンドウは**置き場所に据えていた間**（操作の記録の `place` から次の `move`/`stop` まで）
    の中だけで刻み、跨がない。置き場所が変われば別の場所を測っているので、跨いで
    平均すると**混ざったものが 1 つの実測値として本体へ入る**。移動中はどのウィンドウにも
    入れない。

    🔑 **区切りはサンプルではなく操作の記録から決める。** サンプルの側から区切ると
    - 受信が途絶えた末尾がウィンドウにならず、**打ち切りが静かに減る**（§5-4 が避けたい偏り）
    ...
    if not ordered:
        ...
        raise SessionError(
            "受信サンプルが 1 つもないのでウィンドウを作れません（機材の設定を確かめてください）"
        )
    ...
    for slot, placed_at, left_at in _placements(events):
        ...
        # 端数のウィンドウは出さない。**分母が足りない**ので受信率が意味を持たない。
...
class _SeqClock:
    ...
    ⚠️ 受信の遅れ（UART・USB）ぶん番号が 1 つ前後するが、ウィンドウは数十パケットで、
    区切りの端数のウィンドウは出さないので、ウィンドウの中身には効かない。
    """
...
def _check_samples(header: SessionHeader, samples: list[RxSample]) -> None:
    ...
    for s in samples:
        if s.config_id != expected_config:
            # 知らない設定番号のサンプルは、積分ウィンドウも検波方式も引けない＝どの条件で
        ...
        if s.tx_config_id != expected_tx_config:
            # TX の設定が変わった後のサンプル。送信間隔が違えばウィンドウの分母が別物になる。
            raise SessionError(
...
def censored_fraction(windows: list[Window]) -> float:
    """打ち切りになったウィンドウの割合（§6.1-2B＝集計は Field 側の仕事）。

    分母は電波について何か分かったウィンドウだけ（記録の欠落のウィンドウは除く）。
    """
...
# ウィンドウの全部が PC までの間で落ちた行の `note`。値が空なので本体は残差から外す。
...
def body_tx_power_notice(header: SessionHeader) -> str:
    ...
    ⇒ SMA 端の実測出力を人が画面に入れる。設定値を入れると、差が全ウィンドウに乗る。
    """
...
def to_csv_rows(header: SessionHeader, windows: list[Window]) -> list[list[object]]:
    """ウィンドウを本体のバッチ CSV の行へ変換する（列の契約は `core/batch_csv_schema`）。
    ...
    **利得と給電線損の割り振り**＝本体の予測は
    ...
    if header.tx.position is None or header.rx.position is None:
        # 位置がサンプルごとに決まる構成（機体で測るステージ）。1 本の経路に畳めない。
        raise SessionError(
