...
どちらも `RADIOSIM_FIELD_RX_SAMPLE` が 1 つ来ないという形でしか現れず、集計では同じ
「欠け」＝打ち切りの根拠になる（`apps/field/aggregate.py`）。⇒ **読み取りの
エラー数を数えて外へ出す**（`ReadStats`）。ここで数えないと、配線の不良が
そのまま「電波が弱かった」という測定結果になる。
⚠️ **この層はしきい値を決めない**＝どれだけ汚れていたら測定を捨てるかは受け入れ
試験（§6.1-3 のステージ0）で決めることで、コードの定数ではない。
...
from apps.field.mavlink.dialect import (
    Dialect,
    decode_payload,
    load_dialect,
    x25_crc,
)
from apps.field.session import RadioSettings, RxSample, SessionError
...
class Message:
    ...
    ⚠️ **`link_seq` は Field のシーケンス番号ではない。** これは MAVLink の
...
def to_rx_sample(message: Message, pc_utc: str, spatial_slot: int = 0) -> RxSample:
    """`RADIOSIM_FIELD_RX_SAMPLE` を受信サンプルへ。
...
def to_radio_settings(
    ...
    """`RADIOSIM_FIELD_CONFIG` を無線設定へ（セッションのヘッダを組むときに使う）。

    **受信感度は引数**＝ファームは自分の感度を知らない（データシートと受け入れ
    試験で決める値）。`meas_dbm` を出さないウィンドウの「感度以下（〜未満）」の文面に
    ...
    return RadioSettings(
        ...
        detector=dialect.label("RADIOSIM_FIELD_DETECTOR", f["detector"]),
        integration_window_us=f["integration_window_us"],
        integration_samples=f["integration_samples"],
        antenna=dialect.label("RADIOSIM_FIELD_ANTENNA", f["antenna"]),
...
def rx_samples(
    ...
    return [
        ...
        for m in messages
        if m.name == "RADIOSIM_FIELD_RX_SAMPLE"
    ]
