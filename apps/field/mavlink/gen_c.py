"""
apps/field/mavlink/gen_c.py
============================
`radiosim_field.xml` から、**ファーム用の C ヘッダ**（`apps/field/firmware/main/
radiosim_field_mavlink.h`）を作る（Field phase 1・増分5）。

    python -m apps.field.mavlink.gen_c           … 作り直す
    python -m apps.field.mavlink.gen_c --check   … 作り直すと一致するかだけ見る
...
from apps.field.mavlink.dialect import Dialect, FieldDef, MessageDef, load_dialect, x25_crc

HEADER_PATH = (
    Path(__file__).resolve().parents[1] / "firmware" / "main" / "radiosim_field_mavlink.h"
)
...
def _c_name(message: MessageDef) -> str:
    return message.name.lower()                  # RADIOSIM_FIELD_TX_PACKET → radiosim_field_tx_packet
    ...
    for field, offset in _offsets(message):
        ...
        if field.count:
            lines.append(
                ...
                f"radiosim_field_put_{acc}(p + {offset} + i * {field.element_size}, "
                f"{cast}m->{field.name}[i]);"
            )
        else:
            lines.append(f"    radiosim_field_put_{acc}(p + {offset}, {cast}m->{field.name});")
    lines.append("}")
    return lines
...
def _decode_fn(message: MessageDef) -> list[str]:
    ...
    for field, offset in _offsets(message):
        ...
        if field.count:
            lines.append(
                f"    for (int i = 0; i < {field.count}; i++) "
                f"m->{field.name}[i] = ({ctype})radiosim_field_get_{acc}(p + {offset} + i * {field.element_size});"
            )
        else:
            lines.append(f"    m->{field.name} = ({ctype})radiosim_field_get_{acc}(p + {offset});")
    lines.append("}")
    return lines
...
def _pack_fn(message: MessageDef) -> list[str]:
    ...
    return [
        ...
        "{",
        f"    {name}_encode_payload(frame + RADIOSIM_FIELD_HEADER_LEN, m);",
        f"    return radiosim_field_frame_finish(frame, link_seq, sysid, compid, MSGID_{upper},",
...
def _golden(message: MessageDef) -> list[str]:
    ...
    return [
        f"static const {name}_t RADIOSIM_FIELD_GOLDEN_MSG_{upper} = {{ {inits} }};",
        f"static const uint8_t RADIOSIM_FIELD_GOLDEN_FRAME_{upper}[{len(frame)}] = {{ {_c_bytes(frame)} }};",
    ]
...
def _selftest(dialect: Dialect) -> list[str]:
    ...
    for number, message in enumerate(_ordered(dialect), start=1):
        ...
        lines += [
            ...
            f"{GOLDEN_COMPID}, &RADIOSIM_FIELD_GOLDEN_MSG_{upper});",
            f"        if (n != sizeof RADIOSIM_FIELD_GOLDEN_FRAME_{upper} || "
            f"memcmp(frame, RADIOSIM_FIELD_GOLDEN_FRAME_{upper}, n) != 0) return {number};",
            f"        if (radiosim_field_frame_parse(RADIOSIM_FIELD_GOLDEN_FRAME_{upper}, n, &msgid, &payload, "
...
def _xml_orders(xml_path: Path | None = None) -> dict[str, list[str]]:
    ...
    from apps.field.mavlink.dialect import DIALECT_XML
...
def render(dialect: Dialect | None = None) -> str:
    ...
    out.append(f"#define RADIOSIM_FIELD_MAX_PAYLOAD_LEN {longest}")
    out.append(
        "#define RADIOSIM_FIELD_MAX_FRAME_LEN "
        "(RADIOSIM_FIELD_HEADER_LEN + RADIOSIM_FIELD_MAX_PAYLOAD_LEN + RADIOSIM_FIELD_CHECKSUM_LEN)"
    )
    out.append("")
    out.append("/* このダイアレクトの ID なら CRC_EXTRA、知らない ID なら -1。 */")
    out.append("static inline int radiosim_field_crc_extra(uint32_t msgid)")
    out.append("{")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m apps.field.mavlink.gen_c")
    ...
    if args.check:
        if current != text:
            print(f"古くなっています: {HEADER_PATH}（python -m apps.field.mavlink.gen_c）",
                  file=sys.stderr)
            return 1
        return 0
