...
**流れ**
  ...
  2. RX が起動時に送る `TRACER_CONFIG` を待つ。届いたら、雛形＋その設定でヘッダを
     組み、セッションを作り、置き場所 0 に据えたことを記録する
  3. 以後、受信サンプルを追記する。操作（移動開始・設置・終了）は操作の記録へ
  4. RX の設定が途中で変わったら、そこで終える（ヘッダは書き換えない約束なので、
     別の条件のサンプルを同じセッションに入れられない）
...
def header_template() -> dict[str, Any]:
    ...
    RX 側の PC に届かない（TX の UART は遠くにある）ので、全部を書く。
    """
...
def build_header(
    ...
    *,
    ...
    # 全部「別の組」として窓から外れる。
    payload["tx"]["device_id"] = template["tx"]["device_id"].lower()
...
class _Processor:
    ...
    `on_start` は、最初の RX の設定を受けたときにヘッダを返す関数（ライブ＝組んで
    セッションを作る／読み直し＝既存のヘッダを返す）。
    """

    on_start: Callable[[Message, str], SessionHeader]
    dialect: Dialect
    ...
    def process(self, chunk: bytes, pc_utc: str) -> tuple[list[RxSample], bool]:
        ...
        for message in self.stream.feed(chunk):
            if message.name == "TRACER_CONFIG":
                if not _is_rx(message, self.dialect) or self.stopped:
                    continue
                if self.header is None:
                    self.header = self.on_start(message, pc_utc)
                elif self._differs(message):
                    return [], True
                continue
    ...
    def _differs(self, message: Message) -> bool:
        assert self.header is not None
        radio = self.header.rx.radio
        now = to_radio_settings(message, radio.sensitivity_dbm, self.dialect)
        return now != radio or mac(message.fields["device_id"]) != self.header.rx.device_id
...
class Recorder:
    ...
    def __init__(
        ...
        *,
        ...
        self._processor = _Processor(on_start=self._start, dialect=self._dialect)
    ...
    def _start(self, config: Message, pc_utc: str) -> SessionHeader:
        header = build_header(
            self._template,
            config,
            started_utc=pc_utc,
...
def replay(directory: str | Path, dialect: Dialect | None = None) -> Replay:
    ...
    def on_start(config: Message, pc_utc: str) -> SessionHeader:
        if to_radio_settings(config, header.rx.radio.sensitivity_dbm, dialect) != header.rx.radio:
            raise SessionError("生ログの最初の設定がヘッダと一致しません")
        return header

    processor = _Processor(on_start=on_start, dialect=dialect)
    samples: list[RxSample] = []
    pending = list(events)
