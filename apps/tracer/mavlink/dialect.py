"""
apps/tracer/mavlink/dialect.py
==============================
`radiosim_tracer.xml` を読んで、**フレームの解釈そのもの**（フィールドの並び・
サイズ・CRC_EXTRA）を組み立てる（Tracer phase 1・増分3）。
...
DIALECT_XML = Path(__file__).resolve().parent / "radiosim_tracer.xml"
...
class Dialect:
    ...
    def label(self, enum_name: str, value: int) -> str:
        """enum の値 → 語彙（`TRACER_DETECTOR_MEAN` → `"mean"`）。
