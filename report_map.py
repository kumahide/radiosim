...
_TILE_PX        = 256
_BASEMAP_LAYER  = "pale"          # GSI 淡色地図（_fetch_tile が URL に埋める layer_id）
_BASEMAP_SUBDIR = "basemap_pale"  # DEM キャッシュと分離した専用 subdir
    ...
    try:
        ...
        for x in range(x0, x1 + 1):
            for y in range(y0, y1 + 1):
                subdir = os.path.join(infra.CACHE_DIR, _BASEMAP_SUBDIR, str(x))
                path   = os.path.join(subdir, f"{y}.png")
                arr = infra._fetch_tile(_BASEMAP_LAYER, zoom, x, y, subdir, path)
                if arr is not None:
                    tile = Image.fromarray(np.asarray(arr)).convert("RGB")
                    canvas.paste(tile, ((x - x0) * _TILE_PX, (y - y0) * _TILE_PX))
                    any_tile = True

        if not any_tile:           # 全滅 → 地図なし（呼び出し側が注記を出す）
