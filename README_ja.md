...
1. [概要・用途](#概要用途)
2. [Windows バイナリ版（インストール不要）](#windows-バイナリ版インストール不要)
...
## Windows バイナリ版（インストール不要）

Python や依存ライブラリのインストールは不要です。ZIP を展開して実行するだけで使えます。

### ダウンロード

[GitHub Releases](https://github.com/kumahide/radiosim/releases) から最新版の `RadioSimPro.zip` をダウンロードしてください。

### 展開・起動

1. `RadioSimPro.zip` を任意のフォルダに展開します。
2. 展開されたフォルダ内の `RadioSimPro.exe` をダブルクリックします。

### Windows SmartScreen の警告について

署名なしの実行ファイルのため、初回起動時に Windows Defender SmartScreen の警告が表示されることがあります。
...
PyInstaller でビルドした実行ファイルは、一部のウイルス対策ソフトで誤検知されることがあります。検知された場合は除外設定に追加してください。

### 初回起動時の動作

`RadioSimPro.exe` と同じフォルダに以下のディレクトリが自動生成されます。
...
設定ファイル `radiosim_conf.json` も同フォルダに自動生成され、次回起動時に復元されます。
...
- `freq` を省略した場合は共通設定の値が使われます。環境区分・降雨強度・回折モデルは共通設定で一括指定します
