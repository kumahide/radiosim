...
- **連続追加モード**: 選ぶとバッチウィンドウを開き（前面化し）、地図で TX→RX ペアを置くたびにバッチへ 1 行を自動追加・自動リセットします（+行 追加不要）。RF（周波数・利得・アンテナ高）は追加時点のランチャー値で凍結。確定パスは **TX=塗りドット／RX=方位矢じり**＋距離で表示（近接・同一座標でも形状で送受を判別）。バッチの行変更（削除・編集確定・インポート等）は地図へリアルタイム反映。`append_path`/`existing_paths` で連携。
