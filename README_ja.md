...
9. [使い方 — 一括シミュレーション](#使い方--一括シミュレーション)
10. [入力パラメータ詳細](#入力パラメータ詳細)
11. [計算モデル詳細](#計算モデル詳細)
12. [DEM 取得ロジック](#dem-取得ロジック)
13. [保存パッケージ](#保存パッケージ)
14. [プロジェクトファイル（.rsproj）](#プロジェクトファイルrsproj)
15. [アーキテクチャ](#アーキテクチャ)
16. [開発環境のセットアップ](#開発環境のセットアップ)
17. [テスト](#テスト)
18. [既知の制限・注意事項](#既知の制限注意事項)
...
| **一括シミュレーション** | `batch.py` | バッチ表（CSV I/O） |
...
- **マップウィンドウ**（`views/map_window.py`・アプリ唯一のインスタンス／3 モード）: 座標入力／バッチへの連続追加／DEM キャッシュの可視化・事前取得・削除
...
| 座標形式              | 十進度（DD）/ 度分秒（DMS） | 座標の**表示**表記（`coords.py`）→ 下記                        |
...
> **マップウィンドウはメニューに無い**＝ランチャー下部の**「マップウィンドウ」ボタン**から開きます（→ [マップウィンドウ](#マップウィンドウ)）。
