...
11. [保存パッケージ](#保存パッケージ)
12. [プロジェクトファイル（.rsproj）](#プロジェクトファイルrsproj)
13. [アーキテクチャ](#アーキテクチャ)
14. [テスト](#テスト)
...
- **中継経路（ホップ）**: waypoint 列（地点）を source of truth に、区間ごと独立したリンクバジェットを回す再生中継モデル。全体判定は min（最も余裕の少ない区間）で、内訳と両方を出力
...
| `hops.csv` | **1 行 = 1 ホップ**の台帳（バッチの `summary.csv` とは別契約） |
...
| `multihop` | 中継経路の waypoint 列とホップ別 RF |
...
- **節が無い（キーが無い）＝「その窓の情報を持たない」**であって「空の窓」ではありません。したがって読込側は触らず、保存側は前回値を持ち越します（バッチ窓を閉じただけで行が消えたファイルを書かないため）。
- 数値として読めない値（NaN / Inf）は書きません。該当する節は保存せず、UI がその旨を知らせます（規格外の JSON を作らない）。

### 読込の流儀

読込時は開いている窓（バッチ / 条件探索 / 中継経路）を**確認のうえ閉じ、開き直す**形にしています。「窓を開いた時点の値で凍結する」というアプリ全体の方式とちょうど一致するため、開いている窓へ後から流し込む口を各窓に作らずに済みます。
