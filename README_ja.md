...
- **キャッシュ管理モード**: パン/ズーム追従でキャッシュ済み領域を最高精度で色分け表示（緑=5m航空 / 黄=5m写真 / 水色=10m）。ジェスチャ＝ドラッグ=移動 / Ctrl+ドラッグ=DL / Ctrl+Alt+ドラッグ=強制再取得 / Shift+Ctrl+ドラッグ=範囲削除。DL・削除は確認ダイアログあり。実装は `infrastructure.prefetch_tiles` ほかの公開 API。取得済みは再 DL しない（外部サーバー配慮）。全キャッシュ削除は **設定 > 全キャッシュ削除**。
