...
刻みの制約は **2 系統あり、効くのは制度側**（2026-09-21 に引き直した）:
  ...
  (a) モジュール側（データシート 8.4 節・6 章）
      ...
      - 1 回の送信ごとに **50 ms の休止**（9.4 節の **下側の区分** = 920.6-923.4 MHz
        ＝ BW125 CH0-14 / BW250 CH0-13 / BW500 CH0-12）
      ⇒ 刻み I >= ToA + 50 ms + CS 5 ms

      🔴 **上側の区分**（923.5-928.0 MHz ＝ BW125 CH15-37 / BW250 CH14-36 /
      BW500 CH13-30）は別勘定で、(1) サブパケット長が **レジスタの設定によらず
      32 byte** に落ち（Field の空中フレームはちょうど 32 B なので「載る」が、
      ver.1 互換モードの Fixed-block だけは宛先 3 B を内数に数えるため 29 B）、
      (2) 休止が **送信時間の 10 倍**、(3) WOR 使用禁止。(2) は BW500/SF7・32 B の
      悲観側（DE=1・ToA 23.10 ms）で刻みの下限 254.1 ms ＝ 決めた 250 ms を割る。
      ⇒ **チャネルは下側から選ぶ**（この計算も下側を前提にしている）。
      ⚠️ 区分の判定は **中心周波数でなく CH 番号**で行う（BW500 CH13 は中心
      923.4 MHz だが占有帯域の上端が 923.5 を越えるので上側の一覧に入る）。
