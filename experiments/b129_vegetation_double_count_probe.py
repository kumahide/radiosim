...
測るのは 5 つ:
  ...
  ④ **形の違い（本題）**＝**回折は幾何（何点で決まるか）／吸収は長さ（何 m か）**。
     ...
     「そのウィンドウだけ植生」と「そのウィンドウ以外だけ植生」に割れば、2 つの項が
     **同じ面のどこを見ているか**が分離できる。
...
def _report_shape(links: dict[str, Link], targets: list[str], veg_h: float = 10.0) -> None:
    ...
    print("  「そのウィンドウだけ植生」と「そのウィンドウ以外だけ植生」に割って、2 項の出方を見る。")
    ...
    print(f"{'link':22s} {'決定点':>6s} {'積む点':>6s} | "
          f"{'Δ回折':>7s} {'ウィンドウのみ':>7s} {'ウィンドウ以外':>7s} | "
          f"{'Δ植生':>7s} {'ウィンドウのみ':>7s} {'ウィンドウ以外':>7s} | {'重なり':>6s}")
    shares = []
    for lid in targets:
        lk   = links[lid]
