...
_STRINGS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "proj_loaded":          "Project loaded.\nThe path rows, scenario conditions and the relay path are applied when you open those windows.",
        "proj_close_title":     "Open Project",
        "proj_close_confirm":   "Loading a project closes the open windows (multiple paths / scenario / relay path). Unsaved edits in them are lost.\n\nContinue?",
        ...
        "dlg_batch_complete":   "Multiple Paths",
    ...
    "ja": {
        ...
        "proj_loaded":          "プロジェクトを読み込みました。\n複数経路の行・条件探索の条件・中継経路は、それぞれの窓を開いたときに反映されます。",
        "proj_close_title":     "プロジェクトを開く",
        "proj_close_confirm":   "プロジェクトを読み込むと、開いている窓（複数経路／条件探索／中継経路）を閉じます。保存していない編集は失われます。\n\n続けますか？",
