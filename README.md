...
回折損で **±5〜15 dB** 程度です。DEM の水平解像度（5〜10m）と、建物・樹木を個別に持たないモデルの粗さが主因です。**現地調査の要否を判断するためのツール**であり、最終的な回線設計の根拠には使わないでください。
...
HTML レポートをブラウザで開き、Ctrl+P から「PDF として保存」を選んでください。A4 縦 1 枚に収まるよう版面を固定してあるため、追加のソフトは要りません。
