...
>
> ⚠️ インストーラが**ウィザードを 1 画面も出さずに**「エラー 4551: アプリケーション制御ポリシーによってこのファイルがブロックされました」で中止される場合、これは SmartScreen ではなく **Windows のアプリケーション制御による実行拒否**で、続行するボタンがありません（Windows 11 の Smart App Control が有効な PC で起こります。この機能はクリーンインストールした環境でのみ有効になり得ます）。**その場合は上のポータブル版をお使いください。** 詳細は [docs/manual_ja.md](docs/manual_ja.md#インストーラが-windows-に止められたとき) を参照してください。
