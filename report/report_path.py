...
def save_profile_png(
    ...
    # 単独で配られ、台帳のサムネイルからも開けるので、帳票の開示セクションに書くだけでは
    # **図と出典が離れる**（地図で同じ判断をした＝B-133）。字は `core.disclosure` が
    # 単一ソース＝帳票のセクションと同じ 1 本を引く。
...
/* ⚠️ **8px が下限**（B-219）＝これより詰めると帳票の最小字を割る。3.4 ステージ6 は
   7.5px まで詰めていたが、感度表と補足セクションを1セクションへ統合し重複する行を削ったことで
...
def path_sheet_html(
    ...
    # 感度表（3.4 ステージ6）＝既存パイプラインを摂動条件で数回まわし、余裕度[dB]の幅を
...
<div class="cols">
  <div class="col">
    ...
    <table class="info">
      ...
      <tr><td>{i18n.t('html_rain_rate')}</td><td>{params.rain_rate:.0f} mm/h</td></tr>
