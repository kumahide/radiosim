...
# HTML ではそこが折り返し可能点になり、バーや箇条が 1px でも印字幅を超えると
...
# にバラバラ**になる＝実測で 5.7〜7.8px まで落ち、帳票の最小字（開示セクションの 8px）を
# どれも下回っていた。⇒ **図の幅を基準に、載せた後の大きさから逆算する。**
#
# ⚠️ **縮小フィット（per-path・最大 0.82 倍）は勘定に入れない**＝比べる相手である
# 開示セクション（8px）も**同じ `.fit` の中にあって同率で縮む**ので、両者の比は変わらない。
...
#: ⚠️ **下限は開示セクションの 8px**＝帳票でいちばん小さい字。出典はそれ以上でなければ
...
/* 「結果の取扱に関する補足」セクション（3.0a1）。**4 種のシートが同じ 1 本を引く**ので
   `.sheet.path` 等へはスコープしない（クラス名が固有＝連結しても衝突しない）。
   小さく畳んで最下部に置く＝per-path は A4 1 枚の縮小フィットの中に入るため、
   本文を押しのけない字送りにしてある。印刷でセクションが割れないよう break-inside:avoid。 */
...
/* 感度の変動幅（3.4 ステージ6 / B-219 で `.handling` の内側の子セクションへ統合）＝
   見出し「机上のスクリーニング推定」を二重に出さないため、独立した
   `<section>` ではなく `.handling` の末尾（出典の後）に置く `<div>` にした。
   骨格は `.handling` に揃えるが、見出しは 1 段軽い `<h5>`（子セクションの印）。 */
...
/* 実測残差の層別表（3.4 ステージ6）＝標本が 1 件も無いバッチには出さない
...
@media screen{
  /* min-width:max-content ＝ ウィンドウが A4 幅(210mm)より狭くても body が内容幅まで広がり、
     中央寄せシートが左へはみ出して左端が見切れる（水平スクロールで届かない）のを防ぐ。
     広いウィンドウでは width:auto がビューポート幅になり背景は従来どおり全面に出る。 */
...
# 「NG」、中継では「ワースト区間」（中身は OK）を意味していた（B-207）。縦罫線と
...
# 面ごとに書き写すと、次に足した 1 面だけセクションを持たない形になる
...
# ============================================================
# 感度の変動幅（3.4 ステージ6 / B-219 で「結果の取扱に関する補足」へ統合）
# ------------------------------------------------------------
# 🔑 **「無い」と書いた地面反射の行を「幅」に置き換える**のがこのセクションの存在理由
# （ロードマップ 3.4 ステージ4 の注記）。**単独で出さない**＝地面反射の幅だけを図に
...
#
# 🔴 **B-219＝見出しと感度表を「結果の取扱に関する補足」の 1 セクションへ統合した**
...
def _sensitivity_inner_html(
    ...
    `handling_section_html` からだけ呼ぶ（セクションタグ・見出しの重複回避は呼び出し側の責務）。
    """
    if sens is None:
        return ""
    rows: "list[str]" = []
...
def handling_section_html(
    ...
    *,
    ...
    """「結果の取扱に関する補足」セクションの HTML 断片を返す（**4 種のシート共通**）。

    B-219（2026-09-13）で、感度の変動幅（`sens`）をこのセクションの中へ統合した
