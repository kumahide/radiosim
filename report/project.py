...
条件セット／中継経路の waypoint 列。
  ...
  ⛔ **結果・ウィンドウの位置/サイズ/開閉状態も入れない**（保存するのは入力一式であって
     画面の見た目ではない＝「テンプレエディタ化しない」ガード）。

なぜバッチ行を CSV 列名でなく `PathRow` の属性名で持つのか（⑦）
--------------------------------------------------------------
CSV（`core.batch_csv_schema.CSV_COLUMNS`）は**人が Excel で編集する外部契約**で、座標を
...
セクションが無い（`None`）ことの意味
----------------------------
**「そのウィンドウの情報を持たない」**であって「空のウィンドウ」ではない。⇒ 読込側は触らず、
保存側は**前回値を持ち越す**。ここを「閉じているウィンドウ＝空」と解釈すると、
バッチウィンドウを閉じただけで**行が消えたファイルを上書き保存**する（データ喪失）。

条件探索と中継経路の重み（⚠️ 軽く扱わない）
--------------------------------------------
この 2 つは**ウィンドウの中身以外に器が無い**＝`.rsproj` が事実上**唯一の永続化手段**。
...
# スキーマ
# ------------------------------------------------------------
# **ドキュメント全体で整数 1 つ**（セクションごとの版は持たない）。セクション別の版は 3.2
...
# 条件探索のフェーズ（**閉じた集合**）。`ScenarioSpec.mode` の許可値の単一ソース。
...
class ScenarioSpec:
    ...
    値を保存時に数値へ変換しない理由＝条件探索のウィンドウが「値は文字列で持ち、実行時に
...
@dataclass
class ProjectDoc:
    """プロジェクト 1 件＝入力一式。**セクションごとに `None` を許す**（上記の意味）。"""
...
# `null` が入っていても欠損と同じ `None` が返るので、**壊れたファイルが「そのセクションは
# 無い」として読めてしまう**。読めてしまうのが危険なのは、**そのまま上書き保存
# すると壊れていたセクションの中身が消える**から——直したはずのデータ損失と同じクラス。
#
# 🔑 **判断の決め手＝`to_dict` は `null` も裸の配列も書かない**（セクションが None なら
...
def _section(data: dict, key: str) -> "dict | None":
    """既知のセクション（`batch` / `scenario` / `multihop`）を取り出す。

    **キーが無い＝None（セクションが無い）／dict 以外（`null` を含む）＝`ProjectError`。**
    """
...
# 収束に 5 巡かかった（`null` → セクションが消える／`[5]` → 要素が消える／`"None"` に
# 化ける／`mode: 1` が別のフェーズに化ける／`bool`・`NaN`・4300 桁 …）。
#
# 🔑 **効いたのは「手口を禁じるゲート」ではなかった**＝5 巡目に「素の `str()` 禁止」を
...
def to_dict(doc: ProjectDoc) -> dict:
    """`ProjectDoc` → JSON にできる dict。**セクションが None ならキーごと出さない。**"""
    data: dict = {
...
def from_dict(data: dict) -> ProjectDoc:
    ...
    if m is not None:
        ...
        doc.multihop = mh.MultiHopPath(
            ...
            # 中継ウィンドウを通るあいだに次々と綻ぶ:
            #   ① ウィンドウが `topology` を落とし、**星の地点を鎖として計算**する
            #   ② 保存でファイルの値が**鎖へ書き換わる**
            #   ③ ウィンドウに持たせても、ホップ行は `len(地点)-1` と `wp[i]→wp[i+1]` で
            #      作られる＝**星では区間名が実際とずれ、別の区間の RF を編集する**
            # ⇒ **実行できないものを、読めるようにも持てるようにもしない。**
            #
            # ⚠️ 7 巡目に「`to_dict` は star を書けるので、読みだけ拒否すると
            # *自分が書いたファイルを自分で読めない*」としてウィンドウに持たせる道を採ったが、
            # **その非対称は API を直接叩いたときにしか起きない**（ウィンドウは鎖しか作らない）。
            # **理屈上の対称性より、実際に踏む誤りを優先する。**
            #
            # 🔑 star を読めるようにするのは、**集約規則が決まり、ウィンドウがホップ行を
