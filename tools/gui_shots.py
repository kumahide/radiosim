"""tools/gui_shots.py — ウィンドウを起こして PNG に撮る（**画面を見られない環境用**）。

なぜ要るか
----------
2026-08-07、会社から VS Code のトンネルで自宅 PC を操作していて、GUI を目視でき
なかった。`CopyFromScreen`（画面キャプチャ）は **セッションがロックされていると
真っ黒**になるので使えない。⇒ `PrintWindow` は「ウィンドウよ、この DC へ自分を描け」と
...
（[[project-real-world-env-vdi]]）。見切れのゲートは `tests/test_window_fit.py`。
...
def capture(win, path: str) -> str:
    ...
    ⚠️ **`winfo_id()` ではなく `wm_frame()`** を使う＝前者は Tk の中身のウィンドウで、
