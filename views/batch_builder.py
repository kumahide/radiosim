...
from views import frozen_common, theme, title_bar, window_fit
...
class BatchBuilderWindow(_TableMixin, _CsvMixin, _RunMixin, tk.Toplevel):
    ...
    # ⚠️ ID 列（添字 1）の幅は `batch` が単一ソース（B-057）＝ここに数字を書くと
    # **打てる長さと読める長さが食い違う**。9 と書いてあったころ、16 文字まで打てる
    # のに 9 文字しか見えず、`asaminami24` が `asaminami2` として読めた（値は正しい
    # ので、寸法ゲートにも値のゲートにも映らなかった＝B-046 と同じクラス）。
    # 🔴 **欄幅（13）と上限（11）は同じ数にならない**。`width` の単位は `0` の字の幅で、
    # `asaminami24` は **11.67 単位**要る（`a` `m` `n` が `0` より太い）＝「11 文字ぶんの
    # 幅」は 11 文字を収める保証にならず、幅 11 では 150% で切れた（実測 2026-08-12）。
    # ⇒ **幅は上限より広く取る**（13 ＝ 11.67 + 余裕）。
    # 🔑 **原資は座標欄**＝`DISPLAY_WIDTH_CHARS` を 27 → 26 にして浮かせた
    # （2026-08-12・ユーザー指摘「緯度経度欄の右に余剰がある」＝そのとおりだった）。
    # 🟡 **どの字形でも読めるのは 7 文字まで**（`W` / `M` は `0` の 1.83 倍）。11 文字を
    # 字形によらず保証するには ID 欄に **21 単位**要り、その幅は無い（座標欄を必要量
    # まで削っても届かない）。⇒ 残留として ISSUES.md の B-057 に記録。
    # ⛔ **備考列（添字 9）は同じ扱いにできない**＝上限 40 文字を表に出す幅は物理的に
    # 無い。備考は自由文で、切れても「別の備考に読み違える」害が無い＝**識別子では
    # ない**ので、この規則の対象外とする。
    _CW = coords.DISPLAY_WIDTH_CHARS
    _ID_W = 13
    # 判定列（添字 11）に入るのは `"OK"` / `"NG"` / `"ERR"`（`_set_row_verdict`）。
    # 6 は中身の倍で、ID を広げる原資はここから出した（実測 24px・ja/150%）。
    # ⚠️ **3 まで詰めてはいけない**＝`ERR` の 3 文字は大文字で平均字幅より広く、
    # 125% と 150% で切れる（実測 2026-08-12。原資を出した側を新しい見切れにした）。
    # 周波数・利得・距離は見出しが床で 1px も出ない。
    _WIDTHS = [2, _ID_W, _CW, _CW, 6, 6, 8, 7, 7, 11, 9, 4, 2, 2]
