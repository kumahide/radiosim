...
＝**ウィンドウの寸法をリテラルで持ち、中身が増えた日に黙って切れる**：

  - B-002（2.4）…… バッチの日本語見出しが見切れる
  - B-007（2.4）…… ランチャーの「マップウィンドウ」ボタンがウィンドウ外へ（高さ固定 900）
  - I-000（2.5a1）… バッチに水平距離列を足したら右端が見切れる（幅固定 1080）
  - I-023（2.5b2）… フォント統一でランチャーの必要幅が 464px になり詰まる（幅固定 450）
  - I-024（2.5b2）… 条件探索で条件 5 列目がウィンドウ外へ（幅固定 900・**あとから増える**）

そのつど「そのウィンドウだけ」を実測追従に直してきたので、**次のウィンドウ・次の増え方**で必ず
再発した。ここに寄せる目的は行数の削減ではなく、

  1. 測り方（要求サイズ・画面の上限・縮めない）を 1 か所に確定させること
  2. **決めた寸法を `win._fit_size` に残す**こと
     ＝ウィンドウを横断するゲート（tests/test_window_fit.py）が
     「どのウィンドウも中身が収まっている」を**一律に**検査できるようにすること

の 2 点にある。②が肝で、これが無いとウィンドウごとに手書きのテストを足す運用になり、
新しいウィンドウを足した人が書き忘れた時点で穴が空く（＝これまでの状態）。

使い方
------
ウィンドウを組み立てたあと、そして**中身が増減し得る操作のあと**に呼ぶ::
...
--------------------------------------------------
上の「測って合わせる」だけでは、**中身が画面より大きいウィンドウ**を救えない。実機
...
（B-021＝最下段のボタン列が数 px のバーに潰れ、主要機能へ到達できなくなった）。

そこでウィンドウの中身を `scrollable_body()` の受け皿へ入れる。入る間は今までと完全に
同じ見た目で（スクロールバーは出ない）、**入らなくなった時だけ**スクロールバーが
現れて全部に手が届く。**画面高がいくつであっても壊れない唯一の答え**なので、
ウィンドウごとの手当て（ロゴを縮める・余白を削る）はここまでの繋ぎでしかない::
...
入らなかったときに手が届く*）。**ウィンドウから見た口は今までどおり `window_fit` の
...
# 従来はバッチ 80px / 条件探索 90px / ランチャー「画面の 92%」とウィンドウごとにばらけて
...
# しか残らず 72px のタスクバーを賄えない＝ウィンドウの下端 33px が常にタスクバーの裏に入る。
...
# **実測が取れないときの保険**＝実測できるウィンドウでは `decoration_size()` が
...
def window_position(win: "tk.Tk | tk.Toplevel") -> tuple[int, int]:
    ...
    出ていないウィンドウでは 0 を返す**。`fit_to_content` は `mainloop()` より前に走る
    ので、そこで 0 を信じると「原点にある＝はみ出さない」と判定し、**クランプが
    黙って無効になる**（同じ理由でゲートも 6 ウィンドウ中 5 ウィンドウが空振りした＝実装中に踏んだ）。
...
def window_rect(win: "tk.Tk | tk.Toplevel") -> "tuple[int, int, int, int]":
    """ウィンドウが占める矩形 `(左, 上, 右, 下)`（`geometry()` と同じ**装飾枠**の座標系）。

    ⚠️ **未表示のあいだ `winfo_width/height` は 1 を返す**ので、そのときだけ決めた
    寸法（`_fit_size`）で代用する。`window_position` を使う理由はあちらの註
    （`winfo_x/y` は未表示のウィンドウで 0）。

    ⛔ **表示済みのウィンドウで「大きい方」を採ってはいけない**（2026-08-15・B-089）。
    `_fit_size` は**最後に自動調整した寸法**で、**利用者が手で縮めても更新されない**
    ＝大きい方を採ると矩形が右・下へ水増しされる。左・上のサブモニタの境界付近に
    置いたウィンドウでは、その水増し分がプライマリと重なって `host_monitor` がプライマリを
    選び、**手で縮めただけのウィンドウが主画面へ引き戻される**（B-088 で監視に契機を足した
...
def place_within_screen(
    ...
    """ウィンドウが画面から出ていたら、入る位置へ寄せる。決めた `(x, y)` を返す。
    ...
    **右端に寄せて置く**という正常な置き方を毎回崩す。
    ...
    ⚠️ **入っているウィンドウは動かさない**＝測り直し（DPI 変更・画面変更）のたびに
    ...
    無条件に `max(0, …)` を当てると、**別モニタに置かれた正当なウィンドウを主画面へ
    引き戻す**：左・上のモニタは負座標なので必ず 0 へ丸められ、右のモニタは
    `min(x, screen_w - w)` で引き戻される。

    ⇒ **ウィンドウが載っているモニタの矩形の中へ**収める（`host_monitor`）。

    ⚠️ **「主画面の外なら別モニタ」で済ませてはいけない**＝一度そう書いて Codex に
    差し戻された（2026-08-15・2 巡目）。**サブモニタを外したあと `+2200+100` に
    残ったウィンドウ**は*どこにも無い場所*に居るので、別モニタ扱いで放置すると**掴めない
    まま**になり、B-083 の救済をそのまま取り消す。**実在する矩形と重なるか**で
    判定し、**どれとも重ならないウィンドウだけを主画面へ引き戻す。**

    Args:
        area: 収める先のモニタ矩形。省略するとウィンドウの位置から求める。**`fit_to_content`
            は自分が寸法を決めるのに使った矩形をそのまま渡す**（B-087）＝大きさと
            位置が別々のモニタを基準にすると、収めたつもりのウィンドウが溢れる。
    """
    ...
    # ウィンドウがモニタより大きいと上限が下限を下回り得る（下限 `min_h` が画面を超える
...
def screen_size(win: "tk.Misc") -> tuple[int, int]:
    ...
    「開発機の画面で測ると欠陥が見えない」ことに由来する：
      ...
      1. **テストが出荷先の画面を与えられる**＝開発機は WQHD（使える高さ 1350px）
         なので、FHD で溢れるウィンドウもローカルでは永久に緑になる（B-021 が 6 回目まで
         生き延びた理由そのもの）。
...
def _enumerate_monitors() -> "list[tuple[tuple[int, int, int, int], tuple[int, int, int, int] | None]]":
    ...
    **2 つを一緒に返す理由**（2026-08-18・B-084）＝この 2 つは**同じ列挙の 2 つの
    ...
    - `rcMonitor`＝**モニタの原点を知る**ため（B-085＝別モニタに置かれたウィンドウを
      主画面へ引き戻さない）。
...
def work_areas() -> "dict[tuple[int, int, int, int], tuple[int, int, int, int]]":
    ...
    （B-114）。列挙そのもの（`monitors()`）には残るので、ウィンドウの置き場所は従来どおり
...
def monitors(win: "tk.Misc") -> "list[tuple[int, int, int, int]]":
    """ウィンドウを置ける矩形の一覧。**聞けなければプライマリ 1 枚**として答える。
...
def host_monitor(
    ...
    """ウィンドウ `rect`（左, 上, 右, 下）が**いちばん載っているモニタ**の矩形。

    **どのモニタとも重なっていなければプライマリ**を返す＝それが「引き戻すべきウィンドウ」
    （2026-08-15・B-085 の 2 巡目）。⚠️ **「主画面の外だから別モニタ」とは言えない**
    ＝サブモニタを外したあと `+2200+100` に残ったウィンドウは、*どこにも無い場所*に居る。
    そこを別モニタ扱いで放置すると**ウィンドウは掴めないまま**になり、B-083 の救済を取り消す。
    """
    x0, y0, x1, y1 = rect
...
def decoration_size(win: "tk.Tk | tk.Toplevel") -> tuple[int, int]:
    ...
    **なぜ要るか**（B-084）＝`geometry(f"{w}x{h}")` が決めるのは**クライアント
    ...
    ⛔ **ウィンドウから実測しない**（2026-08-18 に一度そう書いて捨てた）＝`winfo_rootx/rooty`
    と `geometry()` の `+x+y` の差は、**同じウィンドウでも読む時点で変わる**（実装中に実測
    ＝同じランチャーが 58px と 137px を返した。メニューバーの有無・WM がまだ確定
    させていない配置が混ざる）。上限の計算に使う値が測るたびに変わると、**ウィンドウの
    ...
    🔴 **DPI は「このウィンドウ」に聞く**（2026-08-24・B-120）＝下の `_decoration_dpi` の註。
    """
    return _decoration_for(_decoration_dpi(win))


def _decoration_for(dpi: int) -> tuple[int, int]:
    """`dpi` での装飾の寸法。**ウィンドウを持たずに聞ける**＝移動元の DPI の分も出せる
...
def _decoration_dpi(win: "tk.Misc") -> int:
    ...
    ないので全体値（`applied_dpi`）が正しく、**装飾は Windows がウィンドウごとに描く**ので
    ウィンドウごとの値が正しい。以前ここは両方を全体値で束ねており、**倍率の違う複数
    モニタ**でだけ B-084（下端がタスクバーの裏）が戻っていた（150% 側のウィンドウの装飾を
    39px と見積もるが実際は 56px ＝クライアント領域を 17px 大きく取る）。

    ⚠️ **ウィンドウの実 DPI へ単純に差し替えると、今度はテストが条件を再現できない**＝
    ...
    **装飾だけ 100% で計算される**（[views/theme](theme.py) の `applied_dpi` の註が
    ...
    ⇒ **全体値が基準にしたウィンドウ（＝ルート）と同じ DPI に居るウィンドウでは全体値をそのまま
    使い、違う DPI に居るウィンドウだけ自分の値を使う。** 実機ではルートと同じモニタに居る
    ウィンドウは両者が一致するので何も変わらず、**別倍率のモニタへ出したウィンドウだけが正される**。
    """
    ...
    return own                      # 別倍率のモニタ＝このウィンドウの枠はこの DPI で描かれる


def usable_area(
    win: "tk.Tk | tk.Toplevel", area: "tuple[int, int, int, int]"
) -> "tuple[int, int, int, int]":
    """モニタ `area` のうち、ウィンドウの**中身（クライアント領域）**を置いてよい矩形。
    ...
    **どちらも DPI で拡大する**のに定数のままだったので、150% ではウィンドウの下端 33px が
...
def _freeze_table_columns(win: "tk.Misc", *, freeze: bool) -> "list[ttk.Treeview]":
    ...
    ttk の Treeview はウィンドウが中身より広いと列を引き伸ばし、**引き伸ばした幅を
    `-width` として持ち帰る**。その結果、次に測ったときの要求幅は「広げられた
    あとの幅」になり、**表の必要幅が一方通行で増え続ける**（ウィンドウを縮められるように
    しても、中身が元の幅を要求しないので戻らない＝I-053 の最後の一片）。

    そこで**作ったときの列幅を覚えておき**、縮む方向に測り直すときだけ戻す。
    ⚠️ 覚えるのは初回の `fit_to_content` 時点＝ウィンドウはまだ中身ぴったりで、引き伸ばし
    は起きていない（あとから列を足すウィンドウが出たら、そのウィンドウの分はここで覚え直される）。

    ⚠️ **幅を戻すだけでは足りない**＝戻した直後に走る再レイアウトで、まだ広い
    ままのウィンドウに合わせて ttk がもう一度引き伸ばす（測る前に元通りになる）。測って
    いるあいだは `stretch` も止め、寸法を決めたあとに `_thaw_table_columns` で
    元に戻す（新しいウィンドウ幅に合わせた引き伸ばしはそこで改めて走る）。
...
def ready(win: "tk.Tk | tk.Toplevel") -> None:
    """**このウィンドウは組み立て終わった**＝以後の測り直しは畳んでよい（I-107）。

    ウィンドウの `__init__` の最後で呼ぶ。⚠️ **呼び忘れても壊れない**（`fit_soon` が
    従来どおり同期で測るだけ＝速くならないが、間違った寸法にはならない）。
    ⇒ 新しいウィンドウを足した人が忘れても、**安全側**に倒れる。
    """
...
def fit_soon(win: "tk.Tk | tk.Toplevel", run: "Any") -> bool:
    ...
    `run` は「このウィンドウの測り直し 1 回ぶん」を行う引数なしの呼び出し可能物
    （ウィンドウごとに下限・加算値が違うので、**何を測るかはウィンドウが持つ**）。畳んだかどうかを
    返す（真＝あとで走る）。

    **なぜ要るか**＝地点を 1 つ足すたびに `fit_to_content` が走り、**1 回 0.22 秒**
    利用者の手が止まる（実測・中継経路）。同型は 3 つ＝中継経路の地点追加・条件探索の
    条件列追加・バッチ表の列幅追従で、**ウィンドウを開くだけでも 3 回連続で走っている**。

    ⚠️ **畳むのは回数であって、追従そのものではない**（B-021 の再発を自分で作らない）。
    アイドルは*次の描画より前*に来るので、利用者から見た「中身が増えたらウィンドウが広がる」は
    1 フレームも遅れない。**測る側から見て遅れないこと**は 2 つで担保する:

      1. **組み立てが終わるまでは同期で測る**（`ready()` を通るまで）＝`__init__`
         から戻った時点で寸法は決まっていないといけない（`_fit_size` / `_fit_need`
         を読む口がウィンドウの外にもある＝置き場所の計算・横断ゲート）。畳んでよいのは
         *出ているウィンドウを利用者が触っている*あいだだけで、そこが 0.22 秒待たされて
         いる当人でもある。
...
def fit_to_content(
    ...
    *,
    ...
    Args:
        ...
        min_h: 下限の高さ。
        extra_w: 要求幅に足す分。`winfo_reqwidth()` に現れない中身があるウィンドウ向け
            （バッチ表は行がキャンバスの中にあり、縦スクロールバーと外周 padding が
            要求幅に載らない）。**この抜け道があるからこそ**、横断ゲートはウィンドウの申告
            （`_fit_need`）と自前の実測の**大きい方**で検査する。
        extra_h: 同上（高さ）。
        grow_only: 真なら**縮めない**（ユーザーが広げたウィンドウを勝手に狭めない）。

    Returns:
        実際に `geometry()` へ渡した `(幅, 高さ)`。同じ値を `win._fit_size` に、
        必要量を `win._fit_need` に残す（横断ゲートが読む唯一の口）。
    """
    # 畳んで待たせていた測り直しは、いまここで済む（I-107）。⚠️ **ウィンドウごとに溜めるのは
    # 1 本だけ**＝どのウィンドウも測り直しの口は 1 つ（`_fit_to_content` / `_fit_refit`）で、
    # 同じウィンドウに別条件の測り直しが並ぶことはない。
    _drop_soon_fit(win)
    # 呼び出しの内容を残す＝DPI 変更などで**同じ条件のまま測り直す**ため
    # （refit_all が使う。ウィンドウごとに min_w / extra_w が違うので、これが無いと
    # 貼り直し側がウィンドウごとの事情を知らないと再測できない）。
    ...
    # ＝バッチのように自前の再測（`_fit_refit`）を持つウィンドウは自分の `grow_only` で
    # ここを呼び直すので、引数では伝わらない。⚠️ `_fit_kwargs` は**ウィンドウ本来の条件**
    # のまま残す（この上書きは 1 回きりで、ウィンドウの約束を書き換えない）。
    ...
    if escape is not None:
        # ⚠️ **前回出したバーを畳んでから測る**（B-074(a)）。下の測定はウィンドウの
        # `winfo_reqwidth()` を読むが、受け皿のバーはウィンドウの中身なので**出たままだと
        # その幅ぶん要求に載る**。すると「入らなくなって出したバー」が次の測定を
        # 太らせ、**入るようになっても幅が戻らない**（実測＝150% で縦バーが出た
        # あと 100% へ戻すと、ウィンドウ幅にバー 1 本分の 12px が残る。高さが戻るのは
        # 横バーが出ていないからで、直っていたわけではない）。
        # 下の `escape.sync(...)` が必要なぶんを測り直して出し直すので、ここで
        # 畳んでも「入らないウィンドウのバーが消える」ことにはならない。
        escape.sync(overflow_v=False, overflow_h=False)
        # 受け皿越しでも「中身がどれだけ要るか」をウィンドウが正しく申告できる状態にする。
        escape.remeasure()
    win.update_idletasks()
    # 🔴 **上限も「ウィンドウが載っているモニタ」から取る**（2026-08-15・B-087）。
    ...
    # 使える高さ 990px に対して 33px はみ出す。**位置の調整では直らない**（ウィンドウ自体が
    ...
    if escape is not None:
        ...
        if pad_w or pad_h:
            # 縦バーが出た分だけウィンドウを広げる（広げられなければ横バーも出る）。
...
def correct_landing(
    ...
    🔴 **なぜ要るか**（2026-08-23・B-119）。別 DPI のモニタへウィンドウを移すと、Tk 8.6 は
    **ウィンドウ枠の厚みを移る前の DPI のまま**持っている。そこへ `fit_to_content` が
    `geometry("602x1197")` と要求すると、返ってくるウィンドウは `596x1197`＝**幅が 6px
    ...
    ウィンドウに持たせて次回の要求に足す形にしたが、**持つと汚染される**＝①利用者や OS が
...
def required_size(win: "tk.Tk | tk.Toplevel") -> tuple[int, int]:
    ...
    ウィンドウが申告した必要量（`_fit_need`）と、ここでの実測（`winfo_req*`）の**大きい方**
    を返す。ゲート側が「何と比べるか」をウィンドウごとに書き直さずに済ませるための口で、
    申告を信じきらないのは**申告そのものが間違っているウィンドウ**（測り忘れ・測り方の
    誤り）を通さないため。

    🔴 **受け皿を先に測り直す**（2026-08-18・B-100）。逃げ道（`scrollable_body`）を
    持つウィンドウでは `winfo_reqwidth()` が**キャンバスの要求幅で頭打ちになる**＝それを
    ...
    **申告も実測も同じ嘘をつくので、大きい方を採っても救われない。**
    ⚠️ これは「ウィンドウが測り忘れている」ことを**ゲートから見えなくする**欠陥だった
    ＝[[feedback-promote-recurring-checks]] の壊れ方③（間違ったものを要求して
    いる）。B-100 は 1 つのウィンドウの話に見えて、**横断ゲートの目そのものが塞がっていた。**
    """
    # 🔴 **溜まっている測り直しを先に流す**（2026-08-24・I-107）＝畳んだ結果を
    # 「まだ測っていないウィンドウ」として読ませないため。ここは横断ゲートの入口でもある。
    flush_fit(win)
    escape: "_ScrollEscape | None" = getattr(win, "_fit_scroll", None)
    if escape is not None:
        escape.remeasure()
...
def refit_all(root: "tk.Tk | tk.Toplevel", *, shrink: bool = False) -> None:
    ...
    貼り直しただけでは字が大きくなった分だけ見切れる。ウィンドウごとの事情（下限・
    ...
    Args:
        shrink: 真なら `grow_only` を**この 1 回だけ**外し、縮む方向にも測り直す。
            **DPI が変わった経路にだけ渡す**（I-053）＝表示スケールを 150% →
            100% へ戻したのにウィンドウが 150% の大きさのまま残るのを直す。
            受け入れる副作用＝**手で広げたウィンドウも既定サイズへ戻る**（スケールを
            変えた瞬間は「ウィンドウの大きさが変わる」ことが期待そのもの）。
            ⚠️ **画面サイズだけが変わった経路には渡さない**＝あちらは上限が動く
            話で、ユーザーが広げたウィンドウを狭める理由にならない（B-022 の復帰は
            `lim_h` 側で既に効く）。
    """
    for win in (root, *toplevels(root)):
        # ウィンドウ自身が再測メソッドを持つならそちらを優先する。`_fit_kwargs` に残る
        # 加算値（バッチのスクロールバー幅）は**呼んだ時点の実測**なので、DPI が
        # 変わるとスクロールバー自体が太って数 px 足りなくなる。ウィンドウの側で測り直せる
        # なら、その方が正しい。
        # 畳んで待っている測り直しは、これから同じウィンドウを測るので捨てる（I-107）。
...
def toplevels(root: tk.Misc) -> "list[tk.Toplevel]":
    ...
    ⚠️ **公開しているのは [views/theme](theme.py) が同じウィンドウの集合を見るため**
    （B-065＝表示環境の監視も「開いているウィンドウぜんぶ」が単位）。ウィンドウの集め方が 2 つに
    割れると、片方に映って片方に映らないウィンドウが出る。
    """
