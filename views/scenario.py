...
置き場の決定（2026-07-25・ユーザー選択）＝**独立したウィンドウ 1 つ**にタブで比較/スイープ
を同居させる。グラフウィンドウ（既に 1000 行超）へ足すと肥り、バッチウィンドウへ足すと「N 本の
独立回線」と「1 本を掘る」というデータモデルが衝突する（設計哲学⑦）。

**レイアウトの原則（2026-07-25 の実機フィードバックで確定）**:
  - 入力（タブ）は**内容の高さに収める**＝タブを expand させると空白だけが伸びる。
  - 結果は **Treeview（固定高＋スクロール）**＝点数が増えてもウィンドウを縦に伸ばさない
    （41 点でも FHD に収まる）。以前は Label に全点を流し込み、点数次第で
    ウィンドウ外へ溢れて保存ボタンまで見切れた。
  - 実行・レポートのボタンは**常に見えるバー**（タブの外・下端固定）に置く。
...
進捗は [views/progress.ProgressPump](views/progress.py) で受け、フェーズの切り替え
（取得 → 計算 → レポート生成）はランナー側の宣言に従うだけ＝**重いフェーズが管轄外に
...
# 結果一覧の見える行数（超えた分はスクロール）。ウィンドウの高さを点数から切り離す。
_RESULT_ROWS = 8
...
class ScenarioWindow(tk.Toplevel):
    ...
    # 凍結バーが 1050px を要求していたので下限が効いておらず、比較条件が 1 列でも
    # ウィンドウは 1070px で右側が丸ごと空いていた（2026-08-01 実機フィードバック）。
    # バーを詰めた今は**中身に追従する**ので、下限は最小サイズ寄りに戻す。
    # ⚠️ その後 B-046（座標欄 21→27 文字）でバーが 870 → 1085px に太り、**再びバーが
    # ウィンドウ幅を決めた**（＝同じ苦情が 2026-08-08 に再来＝B-052）。処方も 1 回目と同じ
    # ＝**バーを細くする**（↻ と 🔒 の説明を案件情報の行へ移した）。⇒ 実測 ja/dd で
    ...
    def __init__(
        ...
        # 座標の表記＝**開いた時点で凍結**（G2 と同じ形・I-070）。このバーは長らく
    ...
    def project_spec(self) -> "project.ScenarioSpec":
        ...
        ⚠️ ここで数値へ変換しない＝このウィンドウは「値は文字列で持ち、実行時に変換する」
    ...
    def apply_project_spec(self, spec: "project.ScenarioSpec") -> None:
        """プロジェクトの条件セットをこのウィンドウへ取り込む（I-061 のバーから呼ばれる）。"""
    ...
    def _apply_spec(self, spec: "project.ScenarioSpec") -> None:
        ...
        params もランチャーへ流し込まれてからこのウィンドウが開くので、両者は一致する）。
        """
    ...
    def _build_frozen_header(self, parent: tk.Misc) -> None:
        """ランチャーから凍結した前提のバー（I-031）。

        **バッチウィンドウと同じ見せ方に揃える**＝どちらも「ランチャーから凍結した
        スナップショットを読み取り専用で見せるバー」という*同じ性質のもの*なのに、
        ...
        # 2 つ（B-046＝各 315px）と同じ行を分け合っていた。その結果**このバーだけで
        # 1085px を要求し、ウィンドウ幅を決めていた**（条件 5 列のグリッドは 900px）＝
        # 2026-08-01 に一度直した「ウィンドウが広すぎる」が B-046 の後に再来した正体
        # （B-052）。移すとバーは 835px になり、**ウィンドウ幅は比較条件の列数で決まる**。
        # ⚠️ **独立した行にはしない**＝行を 1 つ増やすとこのウィンドウは FHD 100% の 990px
        # を超える（実測 1033px＝100% の実機でスクロールが要るウィンドウになる）。
        # ⚠️ この 2 つは**バー 1 つではなく凍結した領域全体に効く**（`↻` は案件情報・
        # 経路・ベース列をまとめて取り込み直す）ので、1 行目に置くほうが実態に近い
        # ＝バッチも `↻` は共通設定と案件情報の両方を取り込む（`_refresh_common…`）。
        # ⚠️ `width=` は付けない＝文字数で幅を固定すると、中身より広いバーを要求する。
        ...
        # このウィンドウが振れない前提（座標と地形の解像度）。**振れる前提は比較タブの
        ...
        # ずれる＝⑧に反する）。
        # ⚠️ 座標が読めない凍結バーはバーの意味を失う（何を固定したのか分からない）。
        # 実測＝スライス D のバーの作り直しで必要幅は 870 → 1009px（高さ 986px は
        ...
        for label_key, var, expand, width in (
            ...
            # **実際に刻んだ結果こそがこのウィンドウで言うべきこと**（点数はその根拠）。
            # ⚠️ 2 欄に割らない＝このウィンドウはバーがウィンドウ幅を決めてはいけない（B-052）。
            # ⚠️ **幅は字から引き、伸びる側に置く**（B-150）＝15 のべた書きは、画素の
            #    縁を名乗る字が増えた時点で足りなくなる（B-149 と同じ切れ方）。ただし
            #    いちばん長い字をそのまま要求するとバーがウィンドウ幅を決めてしまう（B-052）ので、
    ...
    def _snapshot_meta(self) -> dict[str, str]:
        """ランチャーの案件情報を取り込む（**このウィンドウが持つのは常にこの写し**）。"""
    ...
    def _update_meta_label(self) -> None:
        ...
        **なぜ表示するか**（2026-07-26・ユーザー指摘）：案件名とメモはレポートの
        自己同定ヘッダに刻印されるのに、このウィンドウには出ていなかった。しかも実行の
        瞬間にランチャーを読み直していたので、**画面に無い値が成果物に載る**
        （ウィンドウを開いたあとにランチャー側で案件名を変えると、気づく手段が無い）。
        経路と同じく「凍結して見せる」に揃える。
        """
    ...
    def _fit_to_content(self) -> None:
        ...
        幅を固定すると **「条件を追加」で生やした右端の列がウィンドウ外へ出て見えなくなる**
    ...
    def _build(self) -> None:
        # 中身はスクロールの受け皿の中へ（B-023＝B-021② と同一パス）。このウィンドウは
        ...
        # 実行 & 進捗 & レポート（常に見えるバー）
        # ⚠️ バーが 2 段になったぶん、外側の余白を (10, 6) から詰める＝このウィンドウは FHD
        ...
        # 上段: ステータス 1 行（**バーの上**＝4 ウィンドウ共通の位置・I-047）。以前はこの
        # ウィンドウだけバーの*横*にあり、フェーズ名が伸びるたびに実行ボタンが左へ動いた。
        row1 = ttk.Frame(prog_frame)
        row1.pack(fill="x")
        self._prog_label = ttk.Label(row1, text="", anchor="w")
        self._prog_label.pack(side="left", fill="x", expand=True)

        # 下段: 実行バーの型（I-029）＝**バーが左で伸び、実行はバーの右端**。以前はこのウィンドウ
        ...
        # 判定色は theme が出所（ウィンドウごとに書かない）。**表を作った時点で当てる**＝
        # 結果を入れる関数の中だけで当てると、テーマ切替や他ウィンドウとの突合せで
        # 「まだ色が無い」状態ができる。
    ...
    def _on_run(self) -> None:
        ...
        # meta_provider を読み直しており、ウィンドウを開いたあとにランチャーで案件名を
        ...
        # 成果物生成はランナーの RENDER フェーズ＝ワーカースレッドで走る
    ...
    def close_window(self) -> None:
        ...
        （3 つのウィンドウで名前を揃える＝内部ハンドラ名で分岐させない）。"""
        self._on_close()
    ...
    def _on_close(self) -> None:
        ...
        """
        self._pump.stop()
        # Tcl 側に残る参照を切る（B-050）。これを外さないとウィンドウが解放されない。
