...
tk ウィジェットへ色を渡す役と、全ウィンドウの既定フォント・表の余白を決める役を持つ。

フォントについては `ui_font` / `apply_fonts` / `table_style` の docstring を見ること
（2.5b2 / I-023：ウィンドウごとに `("Arial", 8)` `("Arial", 9)` をベタ書きしていたのを、
...
# （＝`TkDefaultFont` / Segoe UI 9pt のまま）。つまり**何も指定しないウィンドウでも
# ラベルと入力欄で字面が揃わない**。さらにランチャー・バッチだけが
# `("Arial", 8)` `("Arial", 9)` をベタ書きしていたため、ウィンドウごとに書体もサイズも
...
# ⚠️ I-054 の対策（PMv2）はバーだけを直したので、**バーが大きく中身が小さい**という
...
# **OS のメニューフォントに合わせる**（バーは OS が同じ元から描くので、揃う）。
...
# も字の大きさが 1px も変わらない**。一方 Windows（Per-Monitor DPI Aware）はウィンドウの枠
# だけを拡大するので、「ウィンドウは大きくなったのに字は小さいまま」になる
# （2026-07-26 のユーザー報告）。
#
# そこで **96dpi 基準のピクセル数を保持し、実際の DPI 倍して当てる**。
# 「起動時の DPI」でなく「今そのウィンドウが載っているモニタの DPI」を見るので、
...
# **インタプリタに 1 組**しかなく、`TkDefaultFont` をウィンドウごとに別の大きさにはできない。
# ⇒ 複数モニタで DPI が違っても、**字の大きさはアプリ全体で 1 つ**にしかできない。
# 追従は「**最後に表示環境が変わったウィンドウ**の DPI へ全体を合わせる」形になる
# （2026-08-12 ユーザー決定＝動かしたウィンドウが正しくなるほうを取る）。
...
# （I-023 のフォント統一で全ウィンドウの日本語が Yu Gothic UI → ＭＳ Ｐゴシックへ落ちて
# いた副作用も、これで同時に解ける。）
...
def apply_fonts(root: tk.Misc, *, dpi: "int | None" = None) -> None:
    """全ウィンドウの既定フォントを sv_ttk の本文フォントに揃え、**DPI に合わせる**。
    ...
    # ⚠️ **ここでしか差し替えない**＝ウィンドウやウィジェットごとに書体を選ぶと、
    # I-023 で潰したはずの「ウィンドウごとにバラバラ」が別の形で戻る。
...
# 来ない状況では永久に明けない**＝実機ではウィンドウが 120ms ごとに 6px ずつ縮み続け、
...
def _pointer_is_down() -> bool:
    """左右どちらかのボタンが押されている＝**利用者がいまウィンドウを掴んでいる**（Windows のみ）。

    🔴 **掴んでいる最中にウィンドウを測り直してはいけない**（2026-08-23・B-119）＝
    タイトルバーでウィンドウを別 DPI のモニタへドラッグしていく途中は、
    ①手が一瞬止まるたびにデバウンスが明けて ②境界をまたいでいる間は
    「載っているモニタ」も DPI も振れる ⇒ **握っているウィンドウが何度も測り直され、
    ...
    設定には従わない。⇒ 入れ替えている利用者は**物理右ボタン**でウィンドウを掴むので、
...
def watch_display(
    ...
    """**表示環境が変わったら**フォントを貼り直し、ウィンドウを測り直させる。

    見るのは 3 つ＝**DPI**・**画面サイズ**・**ウィンドウが載っているモニタ**。どれもウィンドウの
    寸法の入力なので、1 つでも見落とすとその変化が素通りする。⚠️ **3 つ目は
    2026-08-15 に足した**（B-088）＝`screen_size()` は*プライマリ*の値なので、
    **同じ DPI で解像度だけ違うモニタ**へウィンドウを動かしても 1 つ目も 2 つ目も動かない。

    - **DPI**（モニタ間の移動）: Tk 8.6 は `WM_DPICHANGED` を受けて**ウィンドウの
      大きさは**追従させるが、フォントは何もしない（`tk scaling` は起動時の
      スクリーン値で固定）。結果「ウィンドウだけ大きくなって字は小さいまま」になる
    ...
    - **画面サイズ**（解像度変更・VDI の動的解像度・リモート再接続）: `fit_to_content`
      ...
      なればウィンドウがデスクトップの外へ出たまま、広くなればクランプされた小さいまま
      二度と戻らない（`fit_to_content` が二度と呼ばれないため、再接続しても
      直らずアプリの再起動しか回復手段が無かった）。

    ⚠️ **フォントの貼り直しは DPI が変わったときだけ**（解像度だけ変わった時に
    貼り直すのは無駄な上に、全ウィンドウのフォントを触るので副作用の面が広い）。
    測り直しは**どちらの契機でも**呼ぶ。

    `bind_all` で全ウィンドウ分をまとめて拾う＝ウィンドウを1つ足すたびに配線を思い出す
    必要がない（[[feedback-promote-recurring-checks]]：思い出す規則にしない）。

    ⚠️ **測るのはウィンドウごと**（B-065）。旧実装は `<Configure>` を全トップレベルから
    拾いながら、DPI を**常に `root` から**測っていた＝ランチャーを 100% 側に置いた
    まま子ウィンドウだけを 150% 側へドラッグしても値が動かず、何も起きなかった。**契機の
    網は正しく、測る先だけが 1 つに固定されていた**（＝拾えているので気づけない）。
    🔴 **ただし追従できるのは「大きさ 1 つ」まで**＝Tk の名前付きフォントは
    インタプリタに 1 組なので（`_applied_dpi` の註）、**動かしたウィンドウに全体を合わせる**。
    別モニタに残したウィンドウの字も一緒に変わるのは、この作りでは避けられない副作用。

    Args:
        on_change: 表示環境が変わった**あと**に呼ばれる。引数＝`(その時点の DPI,
            DPI が変わったか)`。ウィンドウの寸法を測り直す（`views.window_fit.refit_all`）
    ...
    # ウィンドウごとの前回値。⚠️ **キーはウィジェットでなく Tk のパス名**＝ここはウィンドウより
    # 長生きする状態なので、ウィジェットを掴むと閉じたウィンドウが解放されない（B-050 で
    # 実際に踏んだ形）。消えたウィンドウの分は毎回の走査で落とす。
    seen: "dict[str, tuple]" = {}
    ...
    def _check() -> None:
        ...
        for win in _windows():
            ...
            seen[name] = got
            if prev is None:
                # 初めて見るウィンドウ。**いま字が従っている DPI と違えば契機**＝別 DPI の
                # モニタ側で開かれたウィンドウが、開いた瞬間だけ取り残されるのを防ぐ。
                if dpi != _applied_dpi.get("value", dpi):
                    moved_dpi = dpi
                continue
            if dpi != prev[0]:
                moved_dpi = dpi
                # 🔴 **移動元の DPI はウィンドウごと**（2026-08-23・独立レビュー 42 巡目）。
                # `_applied_dpi["value"]` は**アプリ全体の字が従っている値**なので、
                # 別 DPI のモニタに置いた子ウィンドウを戻す場合は移動元と一致しない
                # （全体が 96 のまま 240 の子ウィンドウを 96 へ戻すと 96 と読めてしまい、
            ...
            if screen != prev[1] or area != prev[2]:
                ...
                # 違うモニタ**へウィンドウをドラッグしても DPI も画面サイズも動かない
                # ＝ウィンドウは広いほうの画面向けの大きさのまま残り、狭い画面で下端が
                # はみ出す（B-087 の直しが**実運用では発火しない**状態だった）。
                screen_changed = True
        for name in set(seen) - alive:
            del seen[name]                          # 閉じたウィンドウを溜めない
    ...
    def _overridden(win: "tk.Tk | tk.Toplevel") -> bool:
        ...
        - **画面が狭くなった**（解像度変更・リモート再接続）: `fit_to_content` は
          ウィンドウを縮めるので、そこへ OS が**元の大きい寸法を戻す**と害が出る＝ウィンドウが
          画面からはみ出す。**「小さい方向だけ」では、この向きが素通りする**
          （＝「解像度変更もまとめて塞いだ」という当初の主張は誤りだった）。

        ⇒ 見るのは向きではなく「**選んだ大きさのままか**」。

        ⚠️ **利用者が変更から 800ms 以内にウィンドウの大きさを変えた場合も、これに当たる**
        （その操作は取り消される）。**受け入れる副作用**＝表示スケールを変えた直後は
        「ウィンドウの大きさが変わる」ことが期待そのもので、既に `refit_all(shrink=True)` が
        同じ理由で「手で広げたウィンドウも既定サイズへ戻す」ことを選んでいる（I-053）。
    ...
    def _settle(args: tuple) -> None:
        ...
        ウィンドウを測り直すと、以後 DPI は変わらないので `_check` は二度と発火せず、
        **再起動するまで直らない**（字だけ大きくなってウィンドウは元のまま＝両方向に
        スクロールバーが出る）。
        ⚠️ **無条件にもう一度やるのではなく、上書きされたときだけ**＝素通りの
        再実行にすると、利用者が変更直後にウィンドウを動かしただけで元へ戻ってしまう。
        """
        state["settle"] = None
...
def table_style(widget: tk.Misc, name: str = "App.Treeview") -> str:
    ...
    try:
        ...
        # 過大にもなった）。結果、**表の行高が DPI 往復で戻らない**＝I-053 でウィンドウを
...
def menu_font(widget: tk.Misc, *, dpi: "int | None" = None) -> tuple:
    ...
    **ピクセル指定（負値）で返す**＝メニューは `tk scaling` を見ないので、pt で
    ...
    基準は `TkMenuFont`＝**OS のメニューフォント**。バーは Windows が同じ元から
    描くので、こうするとバーと中身の大きさが揃う。
    """
    from tkinter import font as tkfont
    ...
    # ⚠️ 既定は **最後に `apply_fonts` が使った DPI**（無ければそのウィンドウの実測値）。
...
def tooltip_options(widget: tk.Misc) -> dict[str, str]:
    ...
    地の色より少し浮かせて「ウィンドウの上に乗った面」に見せる（メニューのアクティブ
    背景と同じ濃淡の作り方）。
    """
    colors = palette(widget)
    return {
...
# 8.8:1（どちらも ng と同等）。レポート HTML と帳票の地図もライトと同じ #bf360c
# （`report/map_graphics.STATUS_RGB`＝B-207 で CSS 側の #e65100 をやめて一本化した。
# #e65100 は白地でも 3.8:1 で、帳票の 9px の字には足りなかった）。
_VERDICT = {
...
def verdict_colors(widget: tk.Misc) -> "dict[str, str]":
    ...
    **ウィンドウごとに色を書かない**ための出所。条件探索だけが緑/赤で中継経路は同色、
    という不揃いが実機フィードバックで出た（レポート側は両方色分けしていたので、
    画面だけが落ちていた＝配色の出所が散っていると片方だけ直る）。

    `err`（成果物の生成に失敗）を引くのは結果を**表の行へ返す**ウィンドウ＝バッチと中継。
...
def apply_verdict_tags(tree: "ttk.Treeview") -> None:
    ...
    **ウィンドウごとに `tag_configure` を書かない**ための口。行を入れる側は
