...
# tkinter は親子で相互参照するので、ウィンドウを閉じても Python 側は**循環ゴミとして残る**。
...
#
#   ウィンドウを 1 枚閉じた後、ワーカーの `gc.collect()` にかかった時間（実測）:
...
#
# **なぜ「ウィンドウを閉じるときに 1 回」ではなく定期なのか**＝地図ウィンドウは閉じても
...
#
# **なぜウィンドウごとに書かないのか**＝それは「思い出す規則」で、ウィンドウを 1 つ足した人が
# 忘れた瞬間に再発する（→ [[feedback-promote-recurring-checks]]）。ルートに 1 つ
# 置けば、**これから増えるウィンドウにも自動で効く**。
#
...
def sweep_tk_garbage(root, *, interval_ms: int = _TK_SWEEP_MS) -> None:
    """閉じたウィンドウの残骸をメインスレッドで回収し続ける（自分で次回を予約する）。
...
# ============================================================
# ワーカーが `root.after(...)` を直に呼ぶと、**取得中にウィンドウを閉じた瞬間**に
...
# 抑えられていたが、**破棄済みのウィンドウへ投函すると `_tkinter` がメインスレッド以外
...
# メインスレッドからの `after` は死んだウィンドウでも**即座に**失敗する（待ちループは
# 非メインスレッドの呼び出しにしか無い）。
#
...
# 投函の待ち行列。**プロセスに 1 本**＝ウィンドウごとに持つと、ウィンドウを足した人が配線を
