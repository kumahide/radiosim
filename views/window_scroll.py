...
def scrollable_body(
    ...
    ウィンドウは以後この戻り値へ組み立てる（`win` へ直接 pack しない）。**中身がウィンドウに入る
    ...
    Args:
        padding: 内側フレームの padding。**受け皿を挟む前にウィンドウが持っていた
...
class _ScrollEscape:
    ...
    こうするとウィンドウの `winfo_reqheight()` は受け皿が無いときと同じ値を返すので、
    `fit_to_content` も横断ゲートも「中身がどれだけ要るか」を今までどおり測れる
    （＝スクロールできることを理由に必要量を小さく偽らない）。ウィンドウが実際にその
    ...
    def __init__(self, win, canvas, body, item, vsb, hsb) -> None:
        ...
        # 束ねればこのウィンドウの中だけに効く**。`bind_all` だと(1)他のウィンドウのホイールまで
        # 拾い(2)後から `bind_all` したウィンドウに**上書きされ**(3)そのウィンドウが閉じるときの
        # `unbind_all` で**こちらの分まで消える**（バッチ表が実際に bind_all して
        # いる）。ウィンドウに閉じたバインドなら、この 3 つがまとめて起きない。
        win.bind("<MouseWheel>", self._on_mousewheel)
        # **ユーザーがウィンドウを縮めたときも**バーを出す。`fit_to_content` は開いたときと
        # 中身が増えたときにしか走らないので、これが無いと「手で小さくしたら下端が
        # 消えて、スクロールもできない」になる（リサイズできるウィンドウ＝グラフウィンドウで露見）。
    ...
    def remeasure(self) -> None:
        ...
        *要求*サイズだけで、ウィンドウが固定サイズなら実際のサイズは動かない
        ＝**要求が伝わらず、ウィンドウは元の幅のまま字だけ大きくなって見切れる**
        ...
        **変化が無ければそこで返る**（2026-08-24・I-106）＝呼び口は
        ...
        `configure` の直後はまだウィンドウが広がっていないので実寸は 1 手遅れて追いつく。
    ...
    def _sync_scrollregion(self) -> None:
        ...
        ⚠️ `bbox("all")` から取らない＝あれは*描かれている*大きさで、ウィンドウが未実現の
    ...
    def _on_canvas_configure(self, event) -> None:
        # ウィンドウのほうが中身より大きいときは中身を引き伸ばす（`fill="both"` や
    ...
    def _on_win_configure(self, event) -> None:
        """ウィンドウの実サイズと必要量を突き合わせてバーを出し入れする。
    ...
    def sync(self, *, overflow_v: bool, overflow_h: bool) -> tuple[int, int]:
        ...
        縦バーを出せばその幅だけ中身の使える幅が減る（＝ウィンドウをそのぶん広げないと
