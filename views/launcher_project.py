...
🔑 **プロジェクトの保持者はランチャー**＝閉じているウィンドウのセクションは前回値を持ち越す
（`project.py` の「セクションが無い＝空ではない」と対）。ここはその窓口。
...
class _ProjectMixin:
    ...
    #
    # 保持者はランチャー。**開いているウィンドウからは現在値を集め、閉じているウィンドウのセクションは
    # 前回値を持ち越す**（ウィンドウを閉じただけで内容が消えたファイルを書かない）。
    # 読込は**ウィンドウを閉じず、開いているウィンドウにはバーで知らせて押されたときだけ差し替える**
    # （I-061・2026-08-07）。凍結方式（開く＝スナップショット／見えている値で
    # 実行する）は保たれる＝バーを押さなければ画面は動かない。
    # ----------------------------------------------------------
    ...
    def _collect_project(self) -> "tuple[project.ProjectDoc, list[str]]":
        ...
        **警告は「保存しなかったセクション」を伝えるためのもの**＝読めない値を黙って
        書いて壊れたファイルを作るくらいなら、そのセクションだけ保存せずに知らせる。
    ...
    def _on_open_project(self) -> None:
        ...
        # **ウィンドウは閉じない**（I-061・2026-08-07 ユーザー決定）。開いているウィンドウには
        # 「取り込める」ことだけ知らせ、**押したときだけ**差し替える。
        # ⚠️ 以前は「確認して閉じ、開き直す」形だった＝凍結方式（開く＝スナップ
        # ショット）とちょうど一致するので流し込み口が要らなかった。作業の流れが
        # 切れる（ウィンドウの位置・スクロール位置・選択が失われる）のが代償で、そちらを
        # 取った。**凍結方式そのものは改版していない**＝ウィンドウが黙って書き換わることは
        # 起きない（バーを押さなければ画面は 1px も動かない）。
        self._project = doc
    ...
    def _offer_project_to_open_windows(self) -> None:
        """開いているウィンドウに「内容を取り込む」バーを出す（I-061）。

        ⛔ **既存の `↻ ランチャーから更新` には載せない**＝あちらは凍結バー（共通
        設定・案件情報）だけを触り、利用者が入れた行・地点には手を出さない。
        そこへ差し替えを足すと「共通設定を更新するつもりで押したら行が消えた」
        という、この項目で直そうとしている事故を自分で作ることになる。

        ⚠️ **セクションを持たないファイルではバーを出さない**＝`None` は「そのウィンドウの情報を
        持たない」という意味（`project.py` の約束）で、空にする指示ではない。
        出してしまうと「取り込む」が**行の全消し**として働く。

        ⚠️ **取り込まないままウィンドウを閉じたら、画面に見えている方（ウィンドウの内容）が残る**
        ＝閉じる処理が今までどおりそのウィンドウのセクションを `self._project` へ持ち越すため。
        凍結方式（見えている値が正）と同じ向きなので、これでよい。
        """
        # 呼ばれるのは読込直後（`self._project` は必ず入っている）だが、**保持者から
        # 取る唯一の口**を通す＝セクションがすべて `None` の空 doc になるだけで、下の
        # `section is None` で全部 continue する（バーは 1 本も出ない＝同じ結果）。
        doc = self._project_doc()
        targets = (
        ...
        for attr, section, take in targets:
            ...
            if win is None:
                continue
            # ⚠️ **前の読込のバーを、セクションの有無にかかわらず先に消す**（Codex 指摘）＝
            # バーが掴んでいるのは*読んだときのプロジェクト*の中身なので、次の
            # プロジェクトを読んだあとに押せると**別のプロジェクトの行が入る**。
            # 「新しいバーを出すときに古いのを壊す」だけでは、**新しいバーが出ない
            # 読込**（そのセクションを持たないプロジェクト）で古いバーが生き残る。
