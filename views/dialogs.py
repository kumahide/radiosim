...
def center_on(parent: tk.Misc, dlg: tk.Toplevel) -> None:
    ...
    大きいウィンドウ（ランチャーは高さ 973px）だと実際に起きる。
...
def notice_bar(parent: tk.Misc, message: str, action: str,
               command) -> ttk.Frame:
    """ウィンドウの上に出す**お知らせのバー**（押したときだけ何かが起きる）。

    なぜダイアログでないか（I-061・2026-08-07 ユーザー決定）
    ------------------------------------------------------
    プロジェクトを読み込んだとき、開いているウィンドウの中身を**黙って入れ替えない**。
    かといってモーダルで問い直すと、読み込みの瞬間にウィンドウの数だけ質問が並ぶ。
    ⇒ **バーで知らせ、押したときだけ取り込む**＝[[project-radiosim]] の凍結方式
    （ウィンドウは開いた時点の値で凍結し、見えている値で実行する）を壊さない形。

    ⛔ **既存の `↻ ランチャーから更新` に相乗りさせない**＝あちらは凍結バー
    （共通設定・案件情報）だけを触り、**利用者が入れた行・地点には手を出さない**。
    そこへ差し替えを足すと、「共通設定を更新するつもりで押したら行が消えた」
    という、この項目で直そうとしている事故を自分で作ることになる。

    バーは**閉じられる**（`×`）＝取り込まないという選択も 1 操作で終わる。
    ⚠️ 呼び出し側は返ってきたバーを保持し、取り込んだら `destroy()` すること
    （消し忘れると「取り込み済みなのに誘い続けるバー」が残る）。
    """
...
def clear_notice(win: tk.Misc) -> None:
    """そのウィンドウのお知らせのバーを（あれば）消す。

    ⚠️ **バーを出す側と別に、消す口が要る**（2026-08-11・Codex 独立レビュー P1）＝
    `show_notice` の中でだけ古いバーを壊していたころは、**新しい読込に出すバーが無い
    ときに古いバーが残った**（プロジェクト A のバーを出したまま、そのセクションを持たない B を
    読むと A のバーが居座り、押すと **A の中身が B へ入る**）。⇒ 消すのは「新しいバーを
...
def show_notice(win: tk.Misc, message: str, action: str, command) -> None:
    """ウィンドウの**いちばん上**にお知らせのバーを出す（同じウィンドウに 2 本並べない）。

    バーは `win._notice_bar` に持たせる＝ウィンドウごとに同じ 4 行を書き写さないため
    （3 ウィンドウに配ると必ずどれかで消し忘れる）。押したら `command` を呼び、
    **バーは自分で消える**（取り込み済みなのに誘い続けるバーを残さない）。
    """
    clear_notice(win)
