...
views/map_picks.py
==================
地図ウィンドウの**地点の指定と経路の描画**（`MapWindow` の Mixin）。
...
class _Selected(NamedTuple):
    ...
    ⚠️ **`pos` は「地図が写した並びでの位置」**であって、ウィンドウの行番号ではない
    （読めない座標の行は写しに出ないので、両者は一致しない）。ウィンドウの側が同じ規則で
    位置を解くところまでが約束で、写しを作る口と解く口は**ウィンドウの中で 1 つ**にして
    ある（`_readable_*`）。地図がウィンドウの内部の番号を知る形にすると、そこがずれた
    ときに**黙って別の点を動かす**＝B-068 / B-102 と同じ壊れ方になる。

    `ident` / `label` / `coord` は**選んだ時点の写し**＝ウィンドウが照合に使う名前（経路
    ID・地点名）、画面へ出す呼び名、移動量を測るための元の位置。
    ⚠️ **照合に使うのは `ident` であって `label` ではない**＝画面の呼び名は
    i18n で変わるので、ウィンドウに画面の字を照合させると訳を直しただけで動かなくなる。
    """
    ...
    ident: str          # ウィンドウの側の名前（経路 ID・地点名）＝書き戻す前の照合用
...
class _PickMixin:
    ...
    def _forget_selection(self) -> "_Selected | None":
        ...
        動かす**——I-098 のコア（素のクリックでは黙って書き換わらない）の裏切りになる。
    ...
    def _drop_selection(self, layer: str) -> None:
        """そのレイヤの選択を**黙って落とす**（ウィンドウの側で並びが変わったとき）。
    ...
    def _select_hint_text(self) -> str:
        ...
        ⛔ **既存キーの意味を変えて断片に転用しない**（B-117）＝2.8 から利用者が
        `lang/*.json` で訳を足せる。完結した案内だったキーを「先頭の 1 文」に
        作り替えると、**旧い訳を持つファイルではその 1 文に案内が丸ごと入ったまま
        後ろに 2 文が足され**、重複した字と未訳の英語が混ざる（外部訳は形式しか
        検査できない＝*意味の変更は検出できない*）。⇒ 断片には**新しいキー**を
        与え、旧キーは英語から**消す**——そうすれば旧い訳は起動時に
        「英語にないキーです」として**利用者に報告される**（黙って壊れない）。
        """
    ...
    def _draw_waypoints(self) -> None:
        ...
        for i, (name, lat, lon) in enumerate(points):
            ...
            # 形だけで役割が読めるようにする（ウィンドウの役割ラベルと同じ意味を地図でも）。
    ...
    def _on_map_click(self, coords: tuple) -> None:
        ...
        if self._mode.get() == "waypoints":
            ...
            # ⚠️ ここでマーカーを**足さない**＝ウィンドウへ入れてから写しを描き直す。
            # 足すだけにすると、ウィンドウ側で地点を削除しても地図に残る（消し方が無い）。
            self._refresh_waypoints()
    ...
    # （黙って書き換えない、という I-098 のコアはそのまま＝役割を明示して選ばせる）。
    ...
    def _pick_menu(self) -> tk.Menu:
        """右クリックメニューの**実体はウィンドウに 1 つ**（2026-08-24・B-121）。
    ...
    def _move_selected(self, sel: "_Selected", lat: float, lon: float) -> None:
        """選択中の点を (lat, lon) へ移す（**書き戻す先はウィンドウ**＝地図は写し）。

        🔴 **書き戻す前に照合する**（B-068 / B-102 と同じ形）＝選んでから動かす
        までのあいだにウィンドウの側で行が消える・並びが変わることがあり、位置だけで
        書き戻すと**黙って別の点を動かす**。ウィンドウが「その位置は違う」と言ったら
        （空文字）選び直してもらう。
        """
        self._selection = None
        moved = False
