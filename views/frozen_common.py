...
# バーに出す項目＝`(i18n の見出しキー, SimParams の属性名)`。**順序も含めて正典**。
RADIO_FIELDS = (
    ("lbl_b_freq",     "freq_mhz"),
    ("lbl_p_tx",       "p_tx"),
...
SELECT_FIELDS = (
    ...
    ("lbl_dem_source",  "dem_source"),   # 3.4 ステージ1（I-147）
)

# バーに出るべき属性の全体（ウィンドウ間の突き合わせの基準）。
...
def display_value(attr: str, value: object) -> str:
    """バーに出す文字列。**選択式の 2 つはキーでなく表示ラベル**を出す。

    ⚠️ ランチャー（source of truth）が `Deygout` / `見通し` と表示しているのに、
    凍結したバーだけ `deygout` / `los` と内部キーで出ると、**同じ値がウィンドウによって
    別の言葉に見える**（⑧）。表示ラベルの出所は i18n の 1 か所。
    """
    # 🔴 **ここで既定を補ってはいけない**（B-140＝B-137 の直しが作った欠陥）＝
    #    この関数は**バーを入力として読み直すウィンドウ**（複数経路）と**基底をそのまま実行へ
    #    渡すウィンドウ**（中継経路＝`run_multihop(path, self._base_params)`）の両方が使う。
    #    段階を持たない基底に既定を差し込むと、**中継経路はバーに「中」と出しながら
    #    固定 N で走る**（バーの申告と実行がずれる）。⇒ **名乗れないものは名乗らない**。
    #    既定が要るのは*バーを入力として使う側*なので、補うのはそのウィンドウの仕事。
    if attr == "resolution" and not value:
        return NOT_APPLICABLE
    # `dem_source`（3.4 ステージ1）＝表示名は i18n ではなく `DemSourceSpec.display_name`
...
def resolution_key(label: str) -> str:
    ...
    ⚠️ バーは**表示ラベル**を持つ（上の `display_value`）ので、実行のために内部キーへ
    戻す必要がある。`env_type` はウィンドウごとに `_env_label_to_key` を持っていたが、
    段階は 3 ウィンドウ（複数経路・中継経路・条件探索）に出るので**ここに 1 つだけ**置く。

    🔑 **読めなかったら既定へ黙って化かさない**＝原文をそのまま返し、値域の検査
    （`config.validate_value`）に落とさせる。既定へ寄せると、**利用者が選んでいない
    段階で実行が通り、しかもそれが画面のどこにも出ない**（このウィンドウの共通設定は
...
def resolution_field_width() -> int:
    ...
    **閉じ括弧が消えた状態で配布物のスクリーンショットまで進んだ**。
    ...
    ウィンドウを建てるたびに引く。⚠️ **全角は 2 つぶん**で数える（`len()` だと日本語で
...
def sampling_field_width() -> int:
    ...
    との違い）＝条件探索のバーは**ウィンドウ幅を決めてはいけない**（B-052）ので、幅の予算が
    「条件 5 列のグリッド」で頭打ちになっている。実測＝いちばん長い字
    （`90000 点 / 画素 20.1 m`＝23 桁）を要求するとバーが 976px になり、グリッドの
    911px を超えて**バーがウィンドウ幅を決めてしまう**。
    ⇒ **要求するのは短いほうの字（等間隔の形）だけ**にし、残りは `expand` で
    受ける（ウィンドウはバーより広いので、実際の描画幅はこれより広い）。
    ⚠️ **数のべた書きに戻さないこと**＝字を変えた日にどれだけ足りないかが分からなくなる。
    ⚠️ **最悪の値で測る**＝点数は天井（5 桁）、刻みは 2 桁（「低」の 20.1m）。
    ⚠️ **全角は 2 つぶん**（→ `_display_width`）。
    """
    from core import simulation as sim   # 遅延＝バーを建てる時だけで足りる
...
def readonly_band(parent: tk.Misc, *, widths: "dict[str, int]",
    ...
    Args:
        widths: 属性名 → 欄の文字数。**ウィンドウごとの事情**（既定は 6）。
        fold: 1 行に並べる欄の数。⚠️ **ウィンドウが決める**＝中継経路は区間表がウィンドウ幅を
            決めるべきウィンドウなので小さく折る（B-053 でバーを 859 → 620px 台へ下げた成果を
            食わないため。ウィンドウ新設時には 6 欄を 1 行に並べて 125%/150% で 2088px を
            要求し、横断ゲートが止めている）。
    """
