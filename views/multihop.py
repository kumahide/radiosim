...
**なぜバッチウィンドウのタブにしないのか**（2026-08-01 ユーザー決定）
...
# 座標欄の幅は `coords` が単一ソース（B-046）＝ウィンドウごとに数字を書かない。
...
class MultiHopWindow(_MapSinkMixin, tk.Toplevel):
    ...
    # 980px のウィンドウが開いていた**＝右に 100px 前後の死んだ余白（2026-08-08 ユーザー
    # 指摘・B-053）。下限は「これ以上細くしない」ためのもので、既定幅を決める場所
    # ではない。⇒ 中身より下に置く（`minsize` も同時に下げること＝あちらが上なら
    # `geometry()` を上書きして効かない）。
    # 🔴 **780 → 640 は B-108 の道連れ**（2026-08-18）＝案件情報のバーを折って幅を
    # 返した瞬間、こんどは**下限がウィンドウ幅を決める側に回った**（en の必要幅 683px に対し
    # 下限 780px＝97px の死んだ余白）。**B-053 で 980 → 780 に下げたとき、下限は
    # 「そのときのバーの要求幅」に合わせていた**ので、バーが痩せると取り残される。
    ...
    def __init__(
        ...
        # 上書きし、下限を下げてもウィンドウが細くならない（B-053 で実際に踏んだ）。
        ...
        if initial_path is not None and initial_path.waypoints:
            ...
            # 対象にはしない。座標はこのウィンドウで編集する値で、凍結バー（案件情報・
    ...
    def _launcher_endpoints(self):
        ...
        ⚠️ **読めない値は静かに空欄にする**＝ウィンドウを開く途中でダイアログは出さない
    ...
    def project_path(self) -> "mh.MultiHopPath | None":
        ...
        `None`＝「このウィンドウの情報を持たない」（project.py の約束）。開いただけのウィンドウが
        空の経路でファイルの中身を上書きしないための区別で、`_collect_path` の
        「読めない値は名前つきで弾く」性質はそのまま使う（保存側は例外を受けて
        **そのセクションだけ保存しない**＝黙って壊れた経路を書かない）。
        """
    ...
    def apply_project_path(self, path: "mh.MultiHopPath") -> None:
        """プロジェクトの経路をこのウィンドウへ取り込む（I-061 のバーから呼ばれる）。
    ...
    def _build(self) -> None:
        ...
        # 実行バー＝**上段にステータス 1 行／下段はバーの右に「実行」**（4 ウィンドウで同じ型＝
    ...
    def _build_frozen_header(self, parent: tk.Misc) -> None:
        ...
        # ⚠️ **このウィンドウだけ 2 行に折る**（2026-08-18・B-108）。欄の行と道具の行を
        # 分けると、バーの要求幅は**両者の max**になる（1 行に並べると和になる）。
        ...
        # 🔒 の意味と ↻ は**凍結した領域の先頭（案件情報のバー）の右側**（条件探索と
        # 同じ置き方＝B-052）。以前は共通設定の 2 行目にあり、`↻`（`width=18` の
        # 文字数固定で 158px）と `🔒 ランチャーの値`（98px）が **6 欄のうち後半 3 欄と
        # 同じ行を分け合って**いた。その結果**このバーだけで 859px を要求し、ウィンドウ幅を
        # 決めていた**（区間表 738px・地点表 605px）。移すとバーは 620px 台になり、
        # **ウィンドウ幅は区間表＝このウィンドウの主たる中身が決める**。
        # ⚠️ この 2 つはバー 1 つではなく**凍結領域全体**に効く（`↻` は案件情報と共通
        # 設定をまとめて取り込む＝`_refresh_from_launcher`）ので、先頭のほうが実態に
        # 近い。⚠️ `width=` は付けない＝文字数で幅を固定すると中身より広いバーを要求
        # する（I-046 で「実行」から外したのと同じ理由）。
        #
        # 🔴 **ただし B-053 で移した先が、こんどはここでウィンドウ幅を決めていた**（B-108）＝
        # 欄 2 つ（各 20 文字）と ↻ と説明を**1 行に並べる**とバーは ja 764 / en 781px を
        # 要求し、区間表（ja 741 / en 647px）を上回る。**共通設定を軽くした分が素直に
        # こちらへ移っていた**＝バーを 1 つ測るだけでは凍結領域の要求幅は下がらない。
        # ⇒ **欄を細くするのではなく、道具を次の行へ折る**（案件名・メモは利用者の
        # 文字列をそのまま見せる欄で、下限を削ると読めなくなる＝B-046 と同じ筋）。
        # このウィンドウは**横が狭く縦に余裕がある**ので、共通設定を 3 欄で折ったのと同じ判断。
        # ⚠️ **複数経路・条件探索は 1 行のまま**＝あちらは主たる中身（共通設定 1157〜
        # 1238px・比較グリッド）がバーより広く、折っても幅は 1px も減らないうえ、条件
        # 探索は高さ 986px で FHD の天井が近い（行を足すほうが害になる）。**バーに出す
        # 項目は正典（`frozen_common`）だが、並べ方はウィンドウごと**＝ここは並べ方の話。
        ...
        # [views/frozen_common](frozen_common.py)（ウィンドウごとに並べると、また片方だけ
        # 増えて気づかれない）。このウィンドウが決めるのは**並べ方だけ**。
        # ⚠️ **3 欄で折る**＝複数経路のように 1 行へ並べるとバーがウィンドウ幅を決めてしまう
        # （B-053 で 859 → 620px 台に下げた成果を食う方向。ウィンドウ新設時にも 6 欄を 1 行に
        # 並べて 125%/150% で 2088px を要求し、横断ゲートが止めている）。**ウィンドウ幅を
    ...
    def frozen_common_keys(self) -> "set[str]":
        ...
        作るときも、バーの項目集合はそのゲートで自動的に縛られる**（それがこのメソッドを
        ウィンドウごとに置く理由＝一覧を 1 か所に書くと、ウィンドウを足した人が登録し忘れる）。
        """
    ...
    def _build_results(self, parent: tk.Misc) -> None:
        ...
        持つウィンドウはその表へ返し、成り立たないウィンドウだけが結果一覧を持つ」。中継は区間表が
        1 行 = 1 結果なので、一覧を持つと**同じ数字がウィンドウの中に 2 か所**できる。
    ...
    def _watch_input(self, var: tk.StringVar) -> None:
        ...
        Tcl コマンドは **C レベルを経由して**変数 → ウィンドウ → 変数と循環するので、
        ウィンドウを掴んだまま登録すると **Python の GC が切れず、閉じたウィンドウが永久に残る**
        （条件探索が実際に踏んで 65 個/開閉で積み上がった穴）。
        ⚠️ **条件探索の処方（id を控えて `_on_close` で外す）はここでは効かない**＝
        地点の追加・削除で**変数そのものが動的に捨てられる**ので、後始末を書く
        場所がウィンドウの閉じ際だけでは足りず、捨てる箇所を数え上げれば必ず漏れる。
    ...
    def _watch_name(self, var: tk.StringVar) -> None:
        ...
        ⚠️ **弱参照なのは `_watch_input` と同じ理由**（B-050＝`self` を掴むとウィンドウが
    ...
    def _render_waypoints(self) -> None:
        ...
        for i, vars_ in enumerate(self._wp_vars):
            ...
            # 確定でこのウィンドウの表記へ整形する（I-060 R3）＝整形されること自体が
    ...
    def _reformat(self, var: tk.StringVar) -> None:
        """座標欄 1 つを、このウィンドウの表記へ整形する（読めなければ原文のまま）。"""
    ...
    def _hop_label_text(self, i: int) -> str:
        ...
        片方だけ書式が動いたときに**同じウィンドウの中で見出しの形が 2 種類**になる。
        """
    ...
    def _refresh_hop_labels(self) -> None:
        ...
        **利用者の入力に従って伸びる**のに、`text` を書き換えるだけではウィンドウに伝わらず、
        伸びたぶんが右へ押し出されて**「判定」列が見切れる**（実測＝区間表が
        647 → 993px に伸びてもウィンドウは 801px のまま）。⚠️ **行を作り直さないことと、
        測り直さないことは別**＝前者は B-073 で正しく、足りないのは後者の 1 手。
        ⚠️ 既定の地点名（`TX`/`R1`/`RX`）では表がウィンドウより狭いままなので**再現しない**
        ...
        if changed:
            ...
            # `grow_only` なので**広がる方向にしか動かない**＝打っている最中にウィンドウが
            # 伸び縮みして揺れることはない。
            try:
                self._fit_to_content()
            except tk.TclError:
                pass                # 破棄途中のウィンドウ（閉じ際に変数が書き換わる経路）
    ...
    def _collect_path(self) -> mh.MultiHopPath:
        ...
        （「入力は DD / DMS のどちらも受ける」という約束が、このウィンドウだけ
        成り立っていなかった）。**このウィンドウが経路を組み立てる唯一の口**なので、
