...
- これで軸に何を選んでも見出しの重複は起きない。**古い予告の文言**（CHANGELOG
  3.0 セクション・公開文書）は歴史的記録として残し、いま在る列の説明だけを実態に直す。
"""
...
# 3.4 ステージ1（I-147）以降は利用者が宣言ファイルで足した DEM ソースの `source_id`
# も入り得る＝`core/dem_sources.py:DemSourceSpec.source_id`）。
# 文字列にした理由＝`elev_ok`（真偽）ではなく `elev_source` にしたのは、3.3 で
# DEM ソースが増える拡張のとき**列そのものの意味を変えずに値の種類だけ増やせる**
# ため（真偽だと 3.3 で意味変更＝規約2の予告が要る改名になる）。3.4 ステージ1 はまさに
# この「値の種類が増える」ケース＝列の意味・書式は変えていないので予告不要。
# どのレイヤーが答えたかまでは持たない（層の粒度は取得日の刻印が別に持つ）。
#
# 🔴 **`Elevation_m` の意味を 3.3 で変えた**（規約2の予告どおり実施・3.2 で
# 告知済み）＝取得失敗（`dem.get_elevation` が `nan` を返した標本）は
# 「0.0」ではなく**空欄**（CSV の空フィールド）を書く。0.0 は「海抜0mの正当な
# 値」の意味に一本化された。空欄は pandas なら NaN・Excel なら空セルとして
# 読める（`core/simulation.py:_save_terrain_csv` が単一の書き手）。
