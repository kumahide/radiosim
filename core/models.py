...
def _vegetation_loss(
    ...
    🔑 **「回折損と二重計上ではないのか」への答え**（B-129・2026-08-27 に測って却下）。
    同じキャノピー面が `obstruction_surface`（回折）と ここ（吸収）の**両方**へ入るので、
    B-033〔環境損失が遮蔽由来の量を持っていた〕と同型に見える。**測ったら別の量だった:**

      - **回折損は幾何で決まる**＝Bullington の等価ナイフエッジは**接線を決める
        1〜2 標本**だけで決まり、**植生が経路のどれだけ続くかに依らない**。
        実測＝植生の帯を経路の 5% → 100% に伸ばしても **26.61 dB のまま**
        （小数第 2 位まで不動）。
      - **植生減衰は長さで決まる**＝同じ掃引で **9.41 → 27.60 dB**。
      - 回折損を決めている標本が*植生ぶんの増分*に占める割合は **0〜29%**。

    ⇒ **吸収と回折は分けられる**＝「どちらか一方にする」処方は当たらない。
    📏 再測は `experiments/b129_vegetation_double_count_probe.py`（ゴールデン 26 本・
