...
core/env_facts.py
==================
「環境事実の収集層」（3.2 ステージ5）。バージョン・設定・直近ログ・DEM キャッシュ統計・
実行環境（OS／frozen／DPI／プロキシ設定の有無）を 1 つの dict にまとめて返す。

なぜ要るか
----------
出所刻印（ステージ7）・診断パッケージ（ステージ9）・旧配置の残骸通知は、いずれも「いまこの
...
⚠️ **座標はここで伏せる**（3.2 ステージ4 で見つかった穴）。`radiosim.log` には
...
def _system_dpi() -> "int | None":
    ...
    ウィンドウごとの実 DPI（`views/theme.py::window_dpi`）とは違い、**ウィンドウを
...
def collect() -> dict:
    ...
    刻印（ステージ7）・診断パッケージ（ステージ9）・旧配置の残骸通知が共有する入口。
